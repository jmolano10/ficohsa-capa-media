...
declare namespace xf = "http://tempuri.org/Middleware/v2/Resources/TransferenciasACH/xq/estadoProcesoConsultaDatosACHEnviado/";

declare function xf:estadoProcesoConsultaDatosACHEnviado($successIndicator as xs:string,
        ...
        <VALIDACION_PROCESO>
    		{
				if($successIndicator = 'SUCCESS') then(
					...
					)else if($txnStatus = ('PENDING','SUCCESS')) then(
					...
					<ESTADO_PROCESO>PENDING</ESTADO_PROCESO>, 
					<VALIDACION_MENSAJE>{ fn:concat('OSB-E013 ',$successIndicator,' - Excepcion de TimeOut') }</VALIDACION_MENSAJE>, 
					<CODIGO_ERROR_PROCESO>OSB-E013</CODIGO_ERROR_PROCESO>, 
...
xf:estadoProcesoConsultaDatosACHEnviado($successIndicator,
	$zeroRecords, 
	$messages,
	$txnStatus,
	$idRejection)
...
declare namespace xf = "http://tempuri.org/Middleware/v2/Resources/ProcesoPrestamoATMFacade/xq/procesoPrestamoATMFacadeOut/";
declare namespace ns0 = "http://www.ficohsa.com.hn/middleware.services/procesaMensajeGenericoSwitchFacadeTypes";

declare function xf:procesoPrestamoATMFacadeOut($procesaMensajeGenericoSwitchFacadeRequest1 as element(ns0:procesaMensajeGenericoSwitchFacadeRequest),
    $successIndicator as xs:string,
    $availableAmount as xs:string)
    as element() {
         <ISO8583>
            <ISO8583_HEADER>{ $procesaMensajeGenericoSwitchFacadeRequest1/ISO8583/ISO8583_HEADER/@* , $procesaMensajeGenericoSwitchFacadeRequest1/ISO8583/ISO8583_HEADER/node() }</ISO8583_HEADER>
            {
                let $ISO8583_XML_CONTENT := $procesaMensajeGenericoSwitchFacadeRequest1/ISO8583/ISO8583_XML_CONTENT
                return
                    <ISO8583_XML_CONTENT>
                        {
                            for $MTI in $ISO8583_XML_CONTENT/MTI
                            return
                                <MTI>{ concat(substring(data($MTI), 1, 2), '10') }</MTI>
                        }
                        {
                        ...
                        {
                         	for $TransmissionDateTime_007 in $ISO8583_XML_CONTENT/TransmissionDateTime_007
                         	return
                        		<TransmissionDateTime_007>{ data($TransmissionDateTime_007) }</TransmissionDateTime_007>
                        }
                        {
                        ...
                        {
                            if (not($availableAmount = '') and number($availableAmount) > 0) then (
                            	<AdditionalAmounts_054>
				{ 
                            		fn:concat(
                            			'1001340C',
                            			fn-bea:pad-left(xs:string(fn-bea:decimal-truncate(xs:decimal($availableAmount), 2) * 100), 12, '0'),
                            			'1002340C',
                            			fn-bea:pad-left(xs:string(fn-bea:decimal-truncate(xs:decimal($availableAmount), 2) * 100), 12, '0')
                            		) 
                            	}
                            	...
                            	<AdditionalAmounts_054>0</AdditionalAmounts_054>
                            )
                        }
                        {
                        ...
                        }
                        {
                            ...
                            return
                                <ReservedForPrivateUse_121>{ data($ReservedForPrivateUse_121) }</ReservedForPrivateUse_121>
                        }
                        {
...
declare variable $availableAmount as xs:string external;

xf:procesoPrestamoATMFacadeOut($procesaMensajeGenericoSwitchFacadeRequest1,
    $successIndicator,
    $availableAmount)
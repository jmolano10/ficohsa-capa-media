...
declare namespace xf = "http://tempuri.org/Middleware/v2/Resources/ConsultaSaldosTarjetaCredito/xq/consultaSaldosTarjetaCreditoPAOut/";
...
declare function xf:consultaSaldosTarjetaCreditoPAOut($outputParameters as element(ns2:OutputParameters)?,
        ...
        <ns0:consultaSaldosTarjetaCreditoResponse>
        {
        	if(not(empty($outputParameters/ns2:PT_SALDOS_TARJETA/ns2:PT_SALDOS_TARJETA_ITEM))) then (
        		...
        		(
        			...
        			for $FCY_MIN_PAYMENT in $rowBase/ns2:FCY_MIN_PAYMENT
        			return
        				<FCY_MIN_PAYMENT>{ data($FCY_MIN_PAYMENT) }</FCY_MIN_PAYMENT>,
        			for $FCY_TOTAL_PAYMENT in $rowBase/ns2:FCY_TOTAL_PAYMENT
        			return
        		...
        		let $FCY_MIN_PAYMENT :=  data($sjConsultaSaldosTarjetaCreditoResponse/FCY_MIN_PAYMENT)
	            let $FCY_TOTAL_PAYMENT := $valPagoContadoBase - data($sjConsultaSaldosTarjetaCreditoResponse/FCY_TOTAL_PAYMENT)
        		return(
                	...
                	if(not(empty(data($sjConsultaSaldosTarjetaCreditoResponse/CREDIT_LIMIT_CCY/text()))))then(
                		<CREDIT_LIMIT_CCY>{ data($sjConsultaSaldosTarjetaCreditoResponse/CREDIT_LIMIT/text()) }</CREDIT_LIMIT_CCY>
                	)else(
                		if(not(empty($rowBase))) then (
                	...
                	<MAX_PAYMENT_DATE>{ string($rowBase/ns3:Column[@name = 'MAX_PAYMENT_DATE']/text()) }</MAX_PAYMENT_DATE>,
		       		...
		       		<ns0:consultaSaldosTarjetaCreditoResponseType>
        				<ns0:consultaSaldosTarjetaCreditoResponseRecordType>
		                	...
		                	{
			                	if(string($rowBase/ns3:Column[@name = 'INTRA_AMOUNT']/text()) ='0.0')then(
...
xf:consultaSaldosTarjetaCreditoPAOut($outputParameters,

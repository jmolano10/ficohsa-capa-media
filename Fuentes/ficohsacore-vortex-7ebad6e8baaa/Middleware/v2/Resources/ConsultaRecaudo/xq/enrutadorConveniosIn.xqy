...
declare namespace xf = "http://tempuri.org/Middleware/v2/Resources/ConsultaRecaudo/xq/enrutadorConveniosIn/";

declare function xf:consultaRecaudoEnrutadorConveniosIn($outputParameters as element(ns1:OutputParameters))
    as element() {
        <ns0:serviceRequest>
        	<requestData>{ fn:string($outputParameters/ns1:MENSAJE) }</requestData>
        </ns0:serviceRequest>
};

declare variable $outputParameters as element(ns1:OutputParameters) external;

xf:consultaRecaudoEnrutadorConveniosIn($outputParameters)
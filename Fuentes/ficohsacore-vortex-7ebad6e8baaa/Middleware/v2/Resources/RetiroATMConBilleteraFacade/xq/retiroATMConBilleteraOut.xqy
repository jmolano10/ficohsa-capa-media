...
(:: pragma bea:global-element-parameter parameter="$retiroATMConBilleteraResponse" element="ns1:retiroATMConBilleteraResponse" location="../../RetiroATMConBilletera/xsd/retiroATMConBilleteraTypes.xsd" ::)
(:: pragma bea:local-element-return type="ns0:procesaMensajeGenericoSwitchFacadeRequest/ISO8583" location="../../ProcesaMensajeGenericoSwitch/xsd/procesaMensajeGenericoSwitchFacadeTypes.xsd" ::)
...
declare namespace xf = "http://tempuri.org/Middleware/v2/Resources/RetiroATMConBilletera/xq/retiroATMConBilleteraOut/";

declare function local:getWalletNumber($wallet as xs:string) 
	as xs:string{
	let $walletSub := fn:substring($wallet, 5, 4)
	return fn:concat("****", $walletSub)
};

declare function xf:retiroATMConBilleteraOut($successIndicator as xs:string, $procesaMensajeGenericoSwitchFacadeRequest as element(ns0:procesaMensajeGenericoSwitchFacadeRequest),
    $retiroATMConBilleteraResponse as element(ns1:retiroATMConBilleteraResponse))
    as element() {
        <ISO8583>
            <ISO8583_HEADER>{ $procesaMensajeGenericoSwitchFacadeRequest/ISO8583/ISO8583_HEADER/@* , $procesaMensajeGenericoSwitchFacadeRequest/ISO8583/ISO8583_HEADER/node() }</ISO8583_HEADER>
            {
                ...
                return
                    <ISO8583_XML_CONTENT>
                        ...
                        {
                            if(upper-case($successIndicator) = 'SUCCESS') then(
                            	for $ReservedForPrivateUse_121 in $ISO8583_XML_CONTENT/ReservedForPrivateUse_121
                            	return
                                	<ReservedForPrivateUse_121>{ fn:concat('Ficohsa;', data($ISO8583_XML_CONTENT/CardAcceptorNameLocation_043), ';', fn-bea:dateTime-to-string-with-format('yyyy-MM-dd HH:mm', data($retiroATMConBilleteraResponse/TRANSACTION_DATE) ), ';', data($ISO8583_XML_CONTENT/SystemTraceAuditNumber_011), ';', data($retiroATMConBilleteraResponse/AUTORIZATION_NUMBER), ';', 'RETIRO BILLETERA;', (data($ISO8583_XML_CONTENT/AmountTransaction_004) div 100), ';', 'BILLETERA ', local:getWalletNumber(data($ISO8583_XML_CONTENT/AdditionalDataPrivate_048)), '; Comprometidos a ser los mejores "GRACIAS POR USAR NUESTROS CAJEROS"') }</ReservedForPrivateUse_121>
                            ) else(
...
declare variable $retiroATMConBilleteraResponse as element(ns1:retiroATMConBilleteraResponse) external;
declare variable $successIndicator as xs:string external;

xf:retiroATMConBilleteraOut($successIndicator, $procesaMensajeGenericoSwitchFacadeRequest,
    $retiroATMConBilleteraResponse)
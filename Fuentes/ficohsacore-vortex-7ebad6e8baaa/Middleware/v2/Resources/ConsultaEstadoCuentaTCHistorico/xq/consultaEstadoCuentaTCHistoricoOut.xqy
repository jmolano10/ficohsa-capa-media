								...
								for $mov in $conTransaccionesHistoricasOut/ns0:RowSet/ns0:Row[ns0:Column[@name = 'numtarjeta']/text() = $card_number]
					     		return(
					     			<ns2:consultaEstadoCuentaTCHistoricoDetalleTarjeta>
					        			...
					        			<TRANSACTION_ID>{ data($mov/ns0:Column[lower-case(@name) = 'numrefer']) }</TRANSACTION_ID>
					        			<COD_AUTORIZATION>{ data($mov/ns0:Column[lower-case(@name) = 'codauth']) }</COD_AUTORIZATION>	
					        			<SECUENCIA>{ data($mov/ns0:Column[lower-case(@name) = 'secuencia_plan']) }</SECUENCIA>		        			
					        		</ns2:consultaEstadoCuentaTCHistoricoDetalleTarjeta>
					        	)
					        }
					        </ns2:consultaEstadoCuentaTCHistoricoRegistroTarjetasAdicionales> 

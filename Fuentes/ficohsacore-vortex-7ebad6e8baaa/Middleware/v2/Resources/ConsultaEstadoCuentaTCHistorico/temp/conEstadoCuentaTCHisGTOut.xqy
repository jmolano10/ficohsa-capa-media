...
(:: pragma bea:global-element-parameter parameter="$conInfoHistoricaOut" element="ns0:OutputParameters" location="../../../BusinessServices/MasterDataGT/conInfoHistoricaTC/xsd/conInfoHistoricaTC_sp.xsd" ::)
(:: pragma bea:global-element-parameter parameter="$conTransaccionesHistoricasOut" element="ns2:OutputParameters" location="../../../BusinessServices/MasterDataGT/conTransaccionesHistoricas/xsd/conTransaccionesHistoricas_sp.xsd" ::)
        		...
        		return(
					...
					<CREDIT_LIMIT>{
						if(data($estadoCuentaTCHist/ns3:saldos/ns3:ArrayOfStrucSaldosEstado[1]/ns3:limiteCredito) != 0)then(
							data($estadoCuentaTCHist/ns3:saldos/ns3:ArrayOfStrucSaldosEstado[1]/ns3:limiteCredito)
						)else(
							data($estadoCuentaTCHist/ns3:saldos/ns3:ArrayOfStrucSaldosEstado[2]/ns3:limiteCredito)
						)
					}</CREDIT_LIMIT>,
							
					<CREDIT_LIMIT_CCY>{
						if(data($estadoCuentaTCHist/ns3:saldos/ns3:ArrayOfStrucSaldosEstado[1]/ns3:limiteCredito) != 0)then(
							local:getMoneda(data($estadoCuentaTCHist/ns3:saldos/ns3:ArrayOfStrucSaldosEstado[1]/ns3:moneda))
						)else(
							local:getMoneda(data($estadoCuentaTCHist/ns3:saldos/ns3:ArrayOfStrucSaldosEstado[2]/ns3:moneda))
						)				
					}</CREDIT_LIMIT_CCY>,			
        		...
        		for $conInfoHistorica in $conInfoHistoricaOut/ns0:RowSet/ns0:Row[1]
	                ...
	                return(
	                     if($controlRowUsd > 0 and  $controlRowLocal >0)then(
							...
							<POINTS_AVAILABLE>{ data($rowInfoHistoricaUsd/ns0:Column[lower-case(@name) = 'puntosacumulados']) }</POINTS_AVAILABLE>,
							...
							if($controlRowUsd >0)then(
								...
								if($controlRowLocal >0)then(
									...
									<POINTS_AVAILABLE>{ data($rowInfoHistoricaLocal/ns0:Column[lower-case(@name) = 'puntosacumulados']) }</POINTS_AVAILABLE>,
									<LCY_PREVIOUS_BALANCE>{ data($rowInfoHistoricaLocal/ns0:Column[lower-case(@name) = 'totbalini']) }</LCY_PREVIOUS_BALANCE>,
									<LCY_PERIOD_BALANCE>{ data($rowInfoHistoricaLocal/ns0:Column[lower-case(@name) = 'saldoalcorte']) }</LCY_PERIOD_BALANCE>,
									<LCY_TOTAL_PAYMENT>{ data($rowInfoHistoricaLocal/ns0:Column[lower-case(@name) = 'actualdue']) }</LCY_TOTAL_PAYMENT>,
									<LCY_MIN_PAYMENT>{ data($rowInfoHistoricaLocal/ns0:Column[lower-case(@name) = 'pagominimo']) }</LCY_MIN_PAYMENT>
			        ...
			        for $conEstadoCuentaTCrow in $conTransaccionesHistoricasOut/ns2:RowSet/ns2:Row
					return(
						<ns1:consultaEstadoCuentaTCTarjetasAdicionales>
		        			<CARD_NUMBER>{ data($conEstadoCuentaTCrow/ns2:Column[lower-case(@name) = 'numtarjeta']) }</CARD_NUMBER>
					        <CARD_HOLDER_NAME>{ data($conEstadoCuentaTCrow/ns2:Column[lower-case(@name) = 'nombretarjetahabiente']) }</CARD_HOLDER_NAME>
					        <ns1:consultaEstadoCuentaTCHistoricoRegistroTarjetasAdicionales>
					     		<ns1:consultaEstadoCuentaTCHistoricoDetalleTarjeta>
					        			<TRANSACTION_DATE>{ fn-bea:dateTime-to-string-with-format('yyyyMMdd',data($conEstadoCuentaTCrow/ns2:Column[lower-case(@name) = 'fecefectiva'])) }</TRANSACTION_DATE>
					        			<DESCRIPTION>{ data($conEstadoCuentaTCrow/ns2:Column[lower-case(@name) = 'descripcion'])}</DESCRIPTION>
					        			<AMOUNT>{ data($conEstadoCuentaTCrow/ns2:Column[lower-case(@name) = 'monto'])}</AMOUNT>
					        			<CURRENCY>{ data($conEstadoCuentaTCrow/ns2:Column[lower-case(@name) = 'codmoneda'])}</CURRENCY>
					        			<TYPE_OF_TRANSACTION>{ data($conEstadoCuentaTCrow/ns2:Column[lower-case(@name) = 'tipomovimiento'])}</TYPE_OF_TRANSACTION>
					        			<ORIGINAL_AMOUNT>{ data($conEstadoCuentaTCrow/ns2:Column[lower-case(@name) = 'montooriginal'])}</ORIGINAL_AMOUNT>
					        			<ORIGINAL_CURRENCY>{ data($conEstadoCuentaTCrow/ns2:Column[lower-case(@name) = 'monedaoriginal'])}</ORIGINAL_CURRENCY>
					        			<TRANSACTION_POST_DATE>{ fn-bea:dateTime-to-string-with-format('yyyyMMdd',data($conEstadoCuentaTCrow/ns2:Column[lower-case(@name) = 'fecposteo'])) }</TRANSACTION_POST_DATE>
					        			<TRANSACTION_CODE>{ data($conEstadoCuentaTCrow/ns2:Column[lower-case(@name) = 'codtxn'])}</TRANSACTION_CODE>
					        			<PLAN_NUMBER>{ data($conEstadoCuentaTCrow/ns2:Column[lower-case(@name) = 'plann']) }</PLAN_NUMBER>	        			
					        	</ns1:consultaEstadoCuentaTCHistoricoDetalleTarjeta>
					        </ns1:consultaEstadoCuentaTCHistoricoRegistroTarjetasAdicionales> 
	        			</ns1:consultaEstadoCuentaTCTarjetasAdicionales>
					  ),

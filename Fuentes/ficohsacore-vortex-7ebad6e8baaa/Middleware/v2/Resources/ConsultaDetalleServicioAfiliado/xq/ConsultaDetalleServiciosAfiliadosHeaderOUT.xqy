...
declare function xf:ConsultaServiciosAfiliadosHeaderOUT($RSPConsultaDetalleServicioAfiliadosSara as element(ns8:GetSiteServicePaymentEntitiesForSaraByIdMessageOut))
    as element(auth:ResponseHeader) {
        <auth:ResponseHeader>
        	<successIndicator>{ calcSuccessIndicator(data($RSPConsultaDetalleServicioAfiliadosSara/ns3:OperationResult/ns12:Value)) }</successIndicator>
        </auth:ResponseHeader>
};
...
xf:ConsultaServiciosAfiliadosHeaderOUT($RSPConsultaDetalleServicioAfiliadosSara)
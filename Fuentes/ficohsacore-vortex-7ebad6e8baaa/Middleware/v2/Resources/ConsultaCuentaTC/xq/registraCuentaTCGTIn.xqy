...
declare function xf:registraCuentaTCHNIn($conDatoCuentaOut as element(ns4:OutputParameters),
    ...
    $countryCode as xs:string, $saldolcy as xs:string, $saldofcy as xs:string,
            ...
            <ns1:PT_CUENTA_TC>
                ...
                <ns1:NEXT_CUTOFF_DATE>{ data($sjconsultaCuentaTCResponse/NEXT_CUTOFF_DATE) }</ns1:NEXT_CUTOFF_DATE>
                {
                	if (data($conFechaMaximaPago/ns6:RowSet[1]/ns6:Row[1]/ns6:Column[@name='FechaMaximaPago']) != '') then (
						<ns1:MAX_PAYMENT_DATE>{ data($conFechaMaximaPago/ns6:RowSet[1]/ns6:Row[1]/ns6:Column[@name='FechaMaximaPago']) }</ns1:MAX_PAYMENT_DATE>
    				) else (
			            	...
			            	return
			            		if(fn-bea:trim($currentBalance) != '') then(
			            			...
			            			<CURRENT_BALANCE>{ $saldolcy }</CURRENT_BALANCE>
			            			 ...
			            			 <CURRENT_BALANCE>{ $saldofcy }</CURRENT_BALANCE>
			            			)
			            		)else()
			            }
			            {
                ...
                {	
					...
					return
	        	        <ns1:RSN_CHARGES>{ fn-bea:trim(string($rsncharges/ns4:Column[@name = 'misc_user_1']/text()))}</ns1:RSN_CHARGES>
 	        	}
 	        	{       
	        	    for $accountHolderLegalId in $conDatoCuentaOut/ns4:RowSet/ns4:Row
				 	...
				 	return
				 	   <ns1:ACCOUNT_HOLDER_LEGAL_ID>{ string($accountHolderLegalId/ns4:Column[@name = 'numero_id']/text()) }</ns1:ACCOUNT_HOLDER_LEGAL_ID>
				}	
				{    
				    for $accountHolderName in $conDatoCuentaOut/ns4:RowSet/ns4:Row
				 	...
				 	return
				 		<ns1:ACCOUNT_HOLDER_NAME>{ string($accountHolderName/ns4:Column[@name = 'nombre_cliente']/text()) }</ns1:ACCOUNT_HOLDER_NAME>
				} 
            </ns1:PT_CUENTA_TC>
        </ns1:InputParameters>
};
...
declare variable $saldofcy as xs:string external;

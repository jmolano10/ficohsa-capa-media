...
(:: pragma bea:global-element-parameter parameter="$conDatoCuenta" element="ns4:OutputParameters" location="../../../BusinessServices/MasterDataGT/conDatoCuenta/xsd/conDatoCuenta_sp.xsd" ::)
(:: pragma bea:global-element-parameter parameter="$conEncabezadoEstadoCuenta" element="ns0:OutputParameters" location="../../../BusinessServices/MasterDataGT/conEncabezadoEstadoCuenta/xsd/conEncabezadoEstadoCuenta_sp.xsd" ::)
(:: pragma bea:schema-type-parameter parameter="$ptEquivalencias" type="ns2:OT_EQUIVALENCIAS_CATALOGOS_OUT" location="../../../BusinessServices/MDW/consultaEquivalenciasCatalogos/xsd/consultaEquivalenciasCatalogos_sp.xsd" ::)
...
(:: pragma bea:global-element-parameter parameter="$conFechaMaximaPago" element="ns6:OutputParameters" location="../../../BusinessServices/ProcesosGT/conFechaMaximaPago/xsd/conFechaMaximaPago_sp.xsd" ::)
(:: pragma bea:global-element-parameter parameter="$consultaSaldoSPMD" element="ns7:OutputParameters" location="../../../../v3/BusinessServices/MasterData/conConsultaSaldoTCRG/xsd/conConsultaSaldosCuentaTC_sp.xsd" ::)
...
declare namespace ns6 = "http://xmlns.oracle.com/pcbpel/adapter/db/sp/conFechaMaximaPago";
...
declare function xf:consultaCuentaTCHNOut($sjconsultaCuentaTCResponse as element(ns5:sjconsultaCuentaTCResponse)?,
    ...
    $conFechaMaximaPago as element(ns6:OutputParameters)?,
    ...
    as element(ns3:consultaCuentaTCResponse) {
        <ns3:consultaCuentaTCResponse>
        {
        	if(not(empty($consultaCuentaTC/ns1:PT_CUENTA_TC/ns1:ACCOUNT_NUMBER))) then (
                ...
                if (data($conFechaMaximaPago/ns6:RowSet[1]/ns6:Row[1]/ns6:Column[@name='FechaMaximaPago']) != '') then (
					<MAX_PAYMENT_DATE>{ data($conFechaMaximaPago/ns6:RowSet[1]/ns6:Row[1]/ns6:Column[@name='FechaMaximaPago']) }</MAX_PAYMENT_DATE>
    			) else (
                    ...
                    <HOLDER_INFORMATION>
                        {
	                        ...
	                        return(
	                        <EDUCATION_LEVEL>{ valorCatalogoAPPToOSB($EDUCATION_LEVEL, 'VISION+GT', '10', $ptEquivalencias) }</EDUCATION_LEVEL>
	                        )
                        }
                        {
	                        let $JOB_TITTLE:= string($HOLDER_INFORMATION/JOB_TITTLE/text())
	                        return(
	                        	<JOB_TITTLE>{ valorCatalogoAPPToOSB($JOB_TITTLE, 'VISION+GT', '11', $ptEquivalencias) }</JOB_TITTLE>
	                        	)
	                        }
                        {
	                        let $SCORE:= string($HOLDER_INFORMATION/SCORE/text())
	                        return(
	                        	<SCORE>{ valorCatalogoAPPToOSB($SCORE, 'VISION+GT', '11', $ptEquivalencias) }</SCORE>
	                        )
                        }
                        <MAIN_INCOME>{ string($HOLDER_INFORMATION/MAIN_INCOME/text()) }</MAIN_INCOME>
                        <SECONDARY_INCOME>{ string($HOLDER_INFORMATION/SECONDARY_INCOME/text()) }</SECONDARY_INCOME>
			            ...
			            {
			            	let $currentBalance := $rowDatosCuenta/ns4:Column[@name = 'billed_bal']/text() - $rowConsultaSaldoSPMD/ns7:Column[@name = 'PagosDia']/text()
			            	return
...
declare variable $conFechaMaximaPago as element(ns6:OutputParameters)? external;

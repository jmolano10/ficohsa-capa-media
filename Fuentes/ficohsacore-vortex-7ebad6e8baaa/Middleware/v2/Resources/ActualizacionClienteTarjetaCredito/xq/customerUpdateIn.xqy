...
declare function xf:customerUpdateIn($actualizacionClienteTarjetaCredito as element(ns1:actualizacionClienteTarjetaCredito),$ptEquivalencias as element())
    as element(ns0:CustomerUpdateL8V2Request) {
        <ns0:CustomerUpdateL8V2Request>
            ...
            { 
            	let $birthDate := normalize-space(string($actualizacionClienteTarjetaCredito/BIRTH_DATE/text()))
            	return
            		if($birthDate != "") then(
            ...
            {
			    let $LEGAL_ID_TYPE := normalize-space(fn:string($actualizacionClienteTarjetaCredito/LEGAL_ID_TYPE/text()))
			    return
			        if($LEGAL_ID_TYPE != "") then(
			                        <AZXGCI-TAX-ID-TYP>{ valorCatalogoOSBToAPP($LEGAL_ID_TYPE, 'VISION+', '4', $ptEquivalencias) }</AZXGCI-TAX-ID-TYP>		
			        )else(
			            <AZXGCI-TAX-ID-TYP xsi:nil="true" />
			        )
			}
			{
			    let $LEGAL_ID := fn:string($actualizacionClienteTarjetaCredito/LEGAL_ID/text())
			    return
			        if($LEGAL_ID != "") then(
			            <AZXGCI-TAX-ID>{ $LEGAL_ID }</AZXGCI-TAX-ID>		
			        )else(
           ...
           {
            	let $SALARY_RANGE :=  normalize-space(data($actualizacionClienteTarjetaCredito/SALARY_RANGE))
            	return
            ...
            { 
            	let $expirationDate := normalize-space(string($actualizacionClienteTarjetaCredito/EXPIRATION_DATE_CLIENT_ID/text()))
            	return
            ...
            { 
            	let $issueDate := normalize-space(string($actualizacionClienteTarjetaCredito/ISSUE_DATE_CLIENT_ID/text()))
            	return
            		if($issueDate != "") then(
            ...
            (:Se agregan nuevos campos NATIONALITY y OTHERS_INCOMES 20/08/2025:)
            {
            ...
            }
            {
				...
				let $NATIONALITY_CLEAN :=	
				    fn:replace(
				    	fn:replace($NATIONALITY, "[Ññ]", "#"),
				        "[ÄËÏÖÜäëïöü]", "#"
				    )
				return
				    if($NATIONALITY_CLEAN != "") then (
				      <AZXGCI-USER-DEMO-2>{ $NATIONALITY_CLEAN }</AZXGCI-USER-DEMO-2>
				    ) else (

        	...
        	{		
        		let $messageListFM := fn:tokenize(data($procesaMensajeGenericoT24/MESSAGE),"@FM")
	             return(
		            ...
		            (if (string-length($messageListFM[8]) >0) then <REFERENCE_NUMBER>{ $messageListFM[8] }</REFERENCE_NUMBER> else ()),
		            (if (string-length($messageListFM[9]) >0) then <EXPIRATION_DATE>{ $messageListFM[9] }</EXPIRATION_DATE> else ()),

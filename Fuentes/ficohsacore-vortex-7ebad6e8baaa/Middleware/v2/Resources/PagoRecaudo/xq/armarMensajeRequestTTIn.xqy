...
(:: pragma bea:schema-type-parameter parameter="$tELLERType" type="ns2:TELLERType" location="../../../../Business_Resources/pagoRecaudos/Resources/XMLSchema_-258408779.xsd" ::)
...
declare namespace xf = "http://tempuri.org/Middleware/v2/Resources/PagoRecaudo/xq/armarMensajeRequestTTIn/";
...
declare function xf:pagoRecaudoArmarMensajeRequestTTIn($requestHeader as element(ns4:RequestHeader),
    ...
    as element(ns3:InputParameters) {
        <ns3:InputParameters>
            ...
            <ns3:INFO_T24>
                {
                    for $nodo in (get-nodes($consultaDetalleContratoRecaudosResponse), get-nodes($tELLERType), get-nodes($status), get-nodes($consultadeparametroscoreResponse))
                    return
                    	<ns3:DATA>
		                    <ns3:NAME>{ fn:local-name($nodo) }</ns3:NAME>
		                    <ns3:VALUE>{ fn:string($nodo/text()) }</ns3:VALUE>
...
xf:pagoRecaudoArmarMensajeRequestTTIn($requestHeader,
    $pagoRecaudo,

                ...
                <ns1:Record>
                    ...
                    { 
                    	if ($type != '0') then (
		                    <ns1:Hmr>
		                        ...
		                        { 
		                    		if ($type = '3') then
		                    			if (data($ingresaAcuerdoPagoRequest/OPTIONS_TO_PAY/LIQUIDATIONS/LIQUIDATION[CURRENCY = 'USD']/AMOUNT) != '') then
		                    				<ns1:Local_AmountPay_LIQ>{data($ingresaAcuerdoPagoRequest/OPTIONS_TO_PAY/LIQUIDATIONS/LIQUIDATION[CURRENCY = 'USD']/AMOUNT)}</ns1:Local_AmountPay_LIQ>
		                        		else(
		                        ...
		                        { 
		                    		if ($type = '1' and (data($ingresaAcuerdoPagoRequest/PROMISE_TO_PAY/PAYMENT_DETAILS/PAYMENT_DETAIL[CURRENCY = 'USD']))) then
		                        		<ns1:Local_Porcentaje_DPM>{ data($ingresaAcuerdoPagoRequest/OPTIONS_TO_PAY/DISCOUNTS/DISCOUNT[CURRENCY = 'USD']/PERCENT) }</ns1:Local_Porcentaje_DPM>
		                        	else ()
		                        }
		                        { 
		                    		if ($type = '3') then
		                        		<ns1:Local_Porcentaje_LIQ>{data($ingresaAcuerdoPagoRequest/OPTIONS_TO_PAY/LIQUIDATIONS/LIQUIDATION[CURRENCY = 'USD']/PERCENT)}</ns1:Local_Porcentaje_LIQ>
		                        	else ()
		                        }
                    ...
                    <ns1:Phone>{ data($ingresaAcuerdoPagoRequest/PHONE_NUMBER) }</ns1:Phone>
                    {
	                    ...
	                    <ns1:Promises>
	                    	...
	                    	{
	                    		if (data($ingresaAcuerdoPagoRequest/PROMISE_TO_PAY/PAYMENT_DETAILS/PAYMENT_DETAIL[CURRENCY = 'USD']/AMOUNT))then
	                    			<ns1:Local_Amount>{ data($ingresaAcuerdoPagoRequest/PROMISE_TO_PAY/PAYMENT_DETAILS/PAYMENT_DETAIL[CURRENCY = 'USD']/AMOUNT) }</ns1:Local_Amount>
	                    		else

...
declare namespace xf = "http://tempuri.org/Middleware/v2/Resources/TransferenciaCuentaTDFacade/xq/transferenciaCuentaTDOut/";
...
declare function xf:transferenciaCuentaTDOut($successIndicator as xs:string,
            ...
            <ISO8583_HEADER>{ $procesaMensajeGenericoSwitchFacadeRequest/ISO8583/ISO8583_HEADER/@* , $procesaMensajeGenericoSwitchFacadeRequest/ISO8583/ISO8583_HEADER/node() }</ISO8583_HEADER>
            {
                ...
                return
                    <ISO8583_XML_CONTENT>
                        ...
                        <AuthorizationIdentificationResponse_038>
							{
								if (upper-case($successIndicator) = 'SUCCESS') then (
                                	let $len := fn:string-length($transactionId)
                                	let $auth := fn:substring($transactionId, ($len - 6) + 1, 6)
                                	return
                                		$auth
                                ) else ( 
									''
...
xf:transferenciaCuentaTDOut($successIndicator,
	$transactionId,

...
(:: pragma bea:global-element-parameter parameter="$outputParameters" element="ns2:cuentasSaldosResponse" location="../../../FPC/CloudCode/Schemas/CloudCodeTypes.xsd" ::)
                ...
                <ns0:consultaActivoResponseRecords>
                    {
                        ...
                        return(
                        	<ns0:consultaActivoResponseRecord>
                                {
                                    for $NUMERO_CUENTA in $CUENTA/ns2:cuenta
                                    return
                                        <ns0:ACCOUNTNUMBER>{ data($NUMERO_CUENTA) }</ns0:ACCOUNTNUMBER>
                                }                                
                                        <ns0:ACCOUNTNAME>{ concat(fn:data($CUENTA/ns2:primerNombre)," ",fn:data($CUENTA/ns2:segundoNombre)," ",fn:data($CUENTA/ns2:primerApellido)," ",fn:data($CUENTA/ns2:segundoApellido)) }</ns0:ACCOUNTNAME>                                                      
                                {
                                    for $MONEDA in $CUENTA/ns2:moneda
                                    return
                                        <ns0:CURRENCY>{ data($MONEDA) }</ns0:CURRENCY>
                                }
                                {
                                    for $SALDO_TOTAL in $CUENTA/ns2:saldoTotal
                                    return

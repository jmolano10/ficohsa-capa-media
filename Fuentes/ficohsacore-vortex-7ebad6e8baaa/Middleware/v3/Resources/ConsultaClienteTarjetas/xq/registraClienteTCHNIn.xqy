...
(:: pragma bea:global-element-parameter parameter="$outputParameters1" element="ns1:OutputParameters" location="../../../../v2/BusinessServices/MasterDataHN/conDatoCliente/xsd/conDatoCliente_sp.xsd" ::)
(:: pragma bea:global-element-parameter parameter="$customerDemographicInquiryL8V2Response1" element="ns2:CustomerDemographicInquiryL8V2Response" location="../../../../v2/BusinessServices/VisionPlusHN/vmx/wsdl/VMX.wsdl" ::)
...
declare namespace xf = "http://tempuri.org/Middleware/v3/Resources/ConsultaClienteTarjetas/xq/registraClienteTCHNIn/";
...
declare function xf:registraClienteTCHNIn($outputParameters1 as element(ns1:OutputParameters),
    $customerDemographicInquiryL8V2Response1 as element(ns2:CustomerDemographicInquiryL8V2Response),
    $outputParameters2 as element(),
            ...
            <ns0:PT_CLIENTE_TARJETAS>
                    {
                        ...
                        return(
        						...
        						fn-bea:serialize(
        							<addresInformationResidence>
        								...
        								<ADDRESS_LINE_4>{ data($demographic/AZXNDO-ADDR-4) }</ADDRESS_LINE_4>
        								{
	        								...
	        								return(
	        									<CITY>{ valorCatalogoAPPToOSB(string($CITY/text()), 'VISION+HN', '7', $outputParameters2) }</CITY>
	        								)
	        							}
       									{
	        								for $STATE in $demographic/AZXNDO-CITY-STATE
	        								return(
	        									<STATE>{ valorCatalogoAPPToOSB(string($STATE/text()), 'VISION+HN', '8', $outputParameters2) }</STATE>
	        								)
       									}
       									{
       										for $COUNTRY in $demographic/AZXNDO-CNTRY-CD
	        								return(
	        									<COUNTRY>{ valorCatalogoAPPToOSB(string($COUNTRY/text()), 'VISION+HN', '9', $outputParameters2) }</COUNTRY>
	        								)
       									}           			
			        		...
			        		return(
			        			<ns0:MARITAL_STATUS>{ valorCatalogoAPPToOSB(string($MARITAL_STATUS/text()), 'MASTERDATAHN', '2', $outputParameters2) }</ns0:MARITAL_STATUS>
			        		),       		
			        		
			        		for $LEGAL_DOCUMENT_TYPE in $demographic/AZXNDO-SSAN-FLAG
			        		return(
			        			<ns0:LEGAL_DOCUMENT_TYPE>{ valorCatalogoAPPToOSB(string($LEGAL_DOCUMENT_TYPE/text()), 'VISION+HN', '4', $outputParameters2) }</ns0:LEGAL_DOCUMENT_TYPE>
			        		),
							...
							return(
								<ns0:TRAVELERS_STATUS>{ valorCatalogoAPPToOSB(string($TRAVELERS_STATUS/text()), 'MASTERDATAHN', '5', $outputParameters2) }</ns0:TRAVELERS_STATUS>
							),				
							...
							return(
								<ns0:EDUCATION_LEVEL>{ valorCatalogoAPPToOSB(string($EDUCATION_LEVEL/text()), 'VISION+HN', '10', $outputParameters2) }</ns0:EDUCATION_LEVEL>
							),
							
							for $GENDER in $demographic/AZXNDO-GENDER-CODE
							return(
								<ns0:GENDER>{ valorCatalogoAPPToOSB(string($GENDER/text()), 'VISION+HN', '1', $outputParameters2) }</ns0:GENDER>				
							),
							...
							<ns0:COUNTRY_ISSUANCE_PASSAPORT>{ data($demographic/AZXNDO-USER-12) }</ns0:COUNTRY_ISSUANCE_PASSAPORT>,	
							<ns0:ACCOUNT_STATUS_INDICATOR>{ data($demographic/AZXNDO-STMT-MSG-2-INDIC) }</ns0:ACCOUNT_STATUS_INDICATOR>,
							<ns0:COUNTRY_2>{ data($demographic/AZXNDO-DL-CNTRY) }</ns0:COUNTRY_2>,
...
declare variable $outputParameters2 as element() external;
...
xf:registraClienteTCHNIn($outputParameters1,

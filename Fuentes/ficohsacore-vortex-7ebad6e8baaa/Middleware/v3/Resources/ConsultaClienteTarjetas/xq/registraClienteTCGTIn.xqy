...
(:: pragma bea:global-element-parameter parameter="$outputParameters1" element="ns1:OutputParameters" location="../../../../v2/BusinessServices/MasterDataGT/conDatoCliente/xsd/conDatoCliente_sp.xsd" ::)
(:: pragma bea:global-element-parameter parameter="$customerDemographicInquiryL8V2Response1" element="ns2:CustomerDemographicInquiryL8V2Response" location="../../../../v2/BusinessServices/VisionPlusGT/vmx/wsdl/VMX.wsdl" ::)
...
declare namespace xf = "http://tempuri.org/Middleware/v3/Resources/ConsultaClienteTarjetas/xq/registraClienteTCGTIn/";
...
declare function xf:registraClienteTCGTIn($outputParameters1 as element(ns1:OutputParameters),
            ...
            <ns0:PT_CLIENTE_TARJETAS>
                    {
                        ...
                        return(
        						...
        						fn-bea:serialize(
        							<addresInformationResidence>
        								...
        								<ADDRESS_LINE_4>{ data($demographic/AZXNDO-ADDR-4) }</ADDRESS_LINE_4>
        								{
	        								...
	        								return(
	        									<CITY>{ valorCatalogoAPPToOSB(string($CITY/text()), 'VISION+GT', '7', $outputParameters2) }</CITY>
	        								)
	        							}
       									{
	        								for $STATE in $demographic/AZXNDO-CITY-STATE
	        								return(
	        									<STATE>{ valorCatalogoAPPToOSB(string($STATE/text()), 'VISION+GT', '8', $outputParameters2) }</STATE>
	        								)
       									}
       									{
       										for $COUNTRY in $demographic/AZXNDO-CNTRY-CD
	        								return(
	        									<COUNTRY>{ valorCatalogoAPPToOSB(string($COUNTRY/text()), 'VISION+GT', '9', $outputParameters2) }</COUNTRY>
	        								)
       									}           			
			        		...
			        		return(
			        			<ns0:MARITAL_STATUS>{ valorCatalogoAPPToOSB(string($MARITAL_STATUS/text()), 'MASTERDATAGT', '2', $outputParameters2) }</ns0:MARITAL_STATUS>
			        		),       		
			        		
			        		for $LEGAL_DOCUMENT_TYPE in $demographic/AZXNDO-SSAN-FLAG
			        		return(
			        			<ns0:LEGAL_DOCUMENT_TYPE>{ valorCatalogoAPPToOSB(string($LEGAL_DOCUMENT_TYPE/text()), 'VISION+GT', '4', $outputParameters2) }</ns0:LEGAL_DOCUMENT_TYPE>
			        		),
							...
							return(
								<ns0:TRAVELERS_STATUS>{ valorCatalogoAPPToOSB(string($TRAVELERS_STATUS/text()), 'MASTERDATAGT', '5', $outputParameters2) }</ns0:TRAVELERS_STATUS>
							),				
							...
							return(
								<ns0:EDUCATION_LEVEL>{ valorCatalogoAPPToOSB(string($EDUCATION_LEVEL/text()), 'VISION+GT', '10', $outputParameters2) }</ns0:EDUCATION_LEVEL>
							),
							
							for $GENDER in $demographic/AZXNDO-GENDER-CODE
							return(
								<ns0:GENDER>{ valorCatalogoAPPToOSB(string($GENDER/text()), 'VISION+GT', '1', $outputParameters2) }</ns0:GENDER>				
							),
...
xf:registraClienteTCGTIn($outputParameters1,

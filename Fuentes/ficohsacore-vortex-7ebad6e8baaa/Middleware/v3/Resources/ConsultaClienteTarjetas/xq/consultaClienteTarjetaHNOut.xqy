...
(:: pragma bea:global-element-parameter parameter="$conDatoCliente" element="ns0:OutputParameters" location="../../../../v2/BusinessServices/MasterDataHN/conDatoCliente/xsd/conDatoCliente_sp.xsd" ::)
(:: pragma bea:global-element-parameter parameter="$customerDemographicInquiryL8V2Response" element="ns2:CustomerDemographicInquiryL8V2Response" location="../../../../v2/BusinessServices/VisionPlusHN/vmx/wsdl/VMX.wsdl" ::)
...
declare namespace xf = "http://tempuri.org/Middleware/v3/Resources/ConsultaClienteTarjetas/xq/consultaClienteTarjetaHNOut/";
...
declare function xf:consultaClienteTarjetaHNOut($conDatoCliente as element(ns0:OutputParameters)?,
    $customerDemographicInquiryL8V2Response as element(ns2:CustomerDemographicInquiryL8V2Response)?,
    ...
    as element(ns1:consultaClienteTarjetasResponse) {
        <ns1:consultaClienteTarjetasResponse>
        {
        	...
        	let $conDatoCliente := $conDatoCliente/ns0:RowSet/ns0:Row[1]
        	let $demographic := $customerDemographicInquiryL8V2Response/DemographicArray/DemographicElement[1]
        	return(
        		<ORG>{ data($conDatoCliente/ns0:Column[@name = 'org']) }</ORG>,
        		<CUSTOMER_ID>{ data($conDatoCliente/ns0:Column[@name = 'numcliente']) }</CUSTOMER_ID>,
        		<addresInformationResidence>
        			...
        			<ADDRESS_LINE_4>{ data($demographic/AZXNDO-ADDR-4) }</ADDRESS_LINE_4>
        			{
	        			...
	        			return(
	        				<CITY>{ valorCatalogoAPPToOSB(string($CITY/text()), 'VISION+HN', '7', $ptEquivalencias) }</CITY>
	        			)
       				}
       				{
	        			for $STATE in $demographic/AZXNDO-CITY-STATE
	        			return(
	        				<STATE>{ valorCatalogoAPPToOSB(string($STATE/text()), 'VISION+HN', '8', $ptEquivalencias) }</STATE>
	        			)
       				}
       				{
	        			for $COUNTRY in $demographic/AZXNDO-CNTRY-CD
	        			return(
	        				<COUNTRY>{ valorCatalogoAPPToOSB(string($COUNTRY/text()), 'VISION+HN', '9', $ptEquivalencias) }</COUNTRY>
	        			)
       				}           			
        		...
        		<customerNames>
        			<GIVEN_NAMES_1>{ data($conDatoCliente/ns0:Column[@name = 'nombre_1']) }</GIVEN_NAMES_1>
        			<GIVEN_NAMES_2>{ data($conDatoCliente/ns0:Column[@name = 'nombre_2']) }</GIVEN_NAMES_2>
        			<GIVEN_NAMES_3>{ data($conDatoCliente/ns0:Column[@name = 'Nombre_3']) }</GIVEN_NAMES_3>
        		</customerNames>,
        		<BIRTH_DATE>{ data($demographic/AZXNDO-DOB) }</BIRTH_DATE>,
        		
        		for $MARITAL_STATUS in $conDatoCliente/ns0:Column[@name = 'estadocivil']
        		return(
        			<MARITAL_STATUS>{ valorCatalogoAPPToOSB(string($MARITAL_STATUS/text()), 'MASTERDATAHN', '2', $ptEquivalencias) }</MARITAL_STATUS>
        		),       		
        		
        		for $LEGAL_DOCUMENT_TYPE in $demographic/AZXNDO-SSAN-FLAG
        		return(
        			<LEGAL_DOCUMENT_TYPE>{ valorCatalogoAPPToOSB(string($LEGAL_DOCUMENT_TYPE/text()), 'VISION+HN', '4', $ptEquivalencias) }</LEGAL_DOCUMENT_TYPE>
        		),
								
				<LEGAL_DOCUMENT_NUMBER>{ data($conDatoCliente/ns0:Column[@name = 'numero_id']) }</LEGAL_DOCUMENT_NUMBER>,
				<FREQUENT_TRAVELERS_NUMBER>{ data($conDatoCliente/ns0:Column[@name = 'ff_nbr']) }</FREQUENT_TRAVELERS_NUMBER>,
				
				for $TRAVELERS_STATUS in $conDatoCliente/ns0:Column[@name = 'bus_id']
				return(
					<TRAVELERS_STATUS>{ valorCatalogoAPPToOSB(string($TRAVELERS_STATUS/text()), 'MASTERDATAHN', '5', $ptEquivalencias) }</TRAVELERS_STATUS>
				),				
				
				<MAIN_EMAIL>{ data($conDatoCliente/ns0:Column[@name = 'email']) }</MAIN_EMAIL>,
				...
				return(
					<EDUCATION_LEVEL>{ valorCatalogoAPPToOSB(string($EDUCATION_LEVEL/text()), 'VISION+HN', '10', $ptEquivalencias) }</EDUCATION_LEVEL>
				),
				
				for $GENDER in $demographic/AZXNDO-GENDER-CODE
				return(
					<GENDER>{ valorCatalogoAPPToOSB(string($GENDER/text()), 'VISION+HN', '1', $ptEquivalencias) }</GENDER>				
				),
...
declare variable $conDatoCliente as element(ns0:OutputParameters)? external;
declare variable $customerDemographicInquiryL8V2Response as element(ns2:CustomerDemographicInquiryL8V2Response)? external;
...
xf:consultaClienteTarjetaHNOut($conDatoCliente,

...
(:: pragma bea:global-element-parameter parameter="$conDatoTarjetaResponse" element="ns0:OutputParameters" location="../../../../v2/BusinessServices/MasterDataGT/conDatoTarjeta/xsd/conDatoTarjeta_sp.xsd" ::)
...
declare function xf:registraMovimientosRecientesTCHNIn($resultGroup as element(*),
        ...
        <ns1:InputParameters>
            <ns1:PT_MOVIMIENTOS_RECIENTES_TC>
                <ns1:PT_MOVIMIENTOS_RECIENTES_TC_ITEM>
        			...
        			{
        				let $localRow := $conDatoTarjetaResponse/ns0:RowSet/ns0:Row[ ns0:Column[@name='CodMoneda']/text() = 'HND' ]
		        		...
		        		return(
			        		if(count($localAccoutnInquiryRSP) > 0 and count($foreignAccoutnInquiryRSP) > 0 ) then (
		        				for $limiteCredito in $foreignAccoutnInquiryRSP/ARXAIO-CRLIM
						        return
						        	if($limiteCredito != '')then(
		        						<ns1:CREDIT_LIMIT>{ data($limiteCredito) }</ns1:CREDIT_LIMIT>
		        					)else(),
		        				for $moneda in $foreignAccoutnInquiryRSP/ARXAIO-ORG
						        return
						     		if($moneda != '')then(
		        						<ns1:CREDIT_LIMIT_CCY>{ data($moneda) }</ns1:CREDIT_LIMIT_CCY>
		        					)else(),        				
		        				for $localBalancePrevio in $localAccoutnInquiryRSP/ARXAIO-LAST-CRLIM
						        return
						        	if($localBalancePrevio != '')then(
		        						<ns1:PREVIOUS_BALANCE_LCY>{ data($localBalancePrevio) }</ns1:PREVIOUS_BALANCE_LCY>	
		        					)else(),
		        				for $foreignBalancePrevio in $foreignAccoutnInquiryRSP/ARXAIO-LAST-CRLIM
						        return
		        				...
		        				for $limiteCredito in $localAccoutnInquiryRSP/ARXAIO-CRLIM
						        return
						        	if($limiteCredito != '')then(
		        						<ns1:CREDIT_LIMIT>{ data($limiteCredito) }</ns1:CREDIT_LIMIT>
		        					)else(),
		        				for $moneda in $localAccoutnInquiryRSP/ARXAIO-ORG
						        return
						        	if($moneda != '')then(
		        						<ns1:CREDIT_LIMIT_CCY>{ data($moneda) }</ns1:CREDIT_LIMIT_CCY>
		        					)else(),
				...
				<ns1:MOVEMENTS>{
					fn-bea:serialize(
		    			...
		    			<MOVIMIENTOS_RECIENTES>
		    			{
							...
							{
        						for $referenceNumber in $mov/ns2:Column[ @name='NumRef' ]
        						return
        					...
        					{
        						for $postingDate in $mov/ns2:Column[ @name='fecposteo' ]
        						return
        							<POSTING_DATE>{ data($postingDate) }</POSTING_DATE>
        					}
        					{
        						for $merchant in $mov/ns2:Column[ @name='merchantstore' ]
        						return
        							<MERCHANT>{ data($merchant) }</MERCHANT>
        					}
        					{
        						for $categoryCode in $mov/ns2:Column[ @name='codcatego' ]
        						return
        							<CATEGORY_CODE>{ data($categoryCode) }</CATEGORY_CODE>
        					}
        					{
								for $secuencia in $mov/ns2:Column[ @name='Secuencia' ]
								return
...
xf:registraMovimientosRecientesTCHNIn($resultGroup,

...
(:: pragma bea:global-element-parameter parameter="$conTransaccionesActualesResponse" element="ns1:OutputParameters" location="../../../../v2/BusinessServices/MasterData/conTransaccionesActuales/xsd/conTransaccionesActuales_sp.xsd" ::)
(:: pragma bea:global-element-parameter parameter="$conDatoTarjetaResponse" element="ns3:OutputParameters" location="../../../../v2/BusinessServices/MasterData/conDatoTarjeta/xsd/conDatoTarjeta_sp.xsd" ::)
...
declare namespace xf = "http://tempuri.org/Middleware/v3/Resources/ConsultaMovimientosRecientesTC/xq/consultaMovimientosRecientesTCOut/";
...
declare function xf:consultaMovimientosRecientesTCOut($resultGroup as element(*)?,
    ...
    as element(ns0:consultaMovimientosRecientesTCReponse) {
        <ns0:consultaMovimientosRecientesTCReponse>
        {
        	if(not(empty($outputParameters/ns4:PT_MOVIMIENTOS_RECIENTES_TC/ns4:PT_MOVIMIENTOS_RECIENTES_TC_ITEM[1]/ns4:OWNER_NAME)))then(
	    	    ...
	    	    let $localRow := $conDatoTarjetaResponse/ns1:RowSet/ns1:Row[ ns1:Column[@name='CodMoneda']/text() = 'NIO']
	    		...
	    		return(
	    			...
	    			)else if(count($localAccoutnInquiryRSP)) then(
	    				for $limiteCredito in $localAccoutnInquiryRSP/AZXAIO-CRLIM
				        return
				        	if($limiteCredito != '')then(
	    						<CREDIT_LIMIT>{ data($limiteCredito) }</CREDIT_LIMIT>
	    					)else(),
	    				for $moneda in $localAccoutnInquiryRSP/AZXAIO-ORG
				        return
				        	if($moneda != '')then(
	    						<CREDIT_LIMIT_CCY>{ data($moneda) }</CREDIT_LIMIT_CCY>
	    					)else(),
...
xf:consultaMovimientosRecientesTCOut($resultGroup,

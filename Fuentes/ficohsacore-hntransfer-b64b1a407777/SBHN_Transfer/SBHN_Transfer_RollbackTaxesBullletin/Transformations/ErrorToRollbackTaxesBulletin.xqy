...
(:: import schema at "../../SBHN_Transfer_Commons/Resources/SBRG_Constants_Commons/Schemas/ErrorMappingRegional.xsd" ::)
...
declare function local:funcErrorToRollbackTaxesBulletin($responseError as element() (:: schema-element(ns1:errorMappingRegionalOutput) ::),
                                                    $codeError as xs:string, 
                                                    $messageError as xs:string, 
                                                    $targetSystem as xs:string, 
                                                    $status as xs:string, 
                                                    $globalId as xs:string) 
                                                    as element() (:: schema-element(ns2:rollbackTaxesBullletinResponse) ::) {
...
local:funcErrorToRollbackTaxesBulletin($responseError, $codeError, $messageError, $targetSystem, $status, $globalId)
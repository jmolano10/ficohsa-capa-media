    ...
    <ns3:CustomerAddL8V2Request>
        {
        	...
        	return(
	            <AZXNAI-LOCAL-ACCT-ORG>330</AZXNAI-LOCAL-ACCT-ORG>,
	            ...
	            return(
		            if($MARITAL_STATUS  != "")then(
		            	<AZXNAI-M-MARITAL-STATUS>{ local:CatalogValueOSBToAPP($MARITAL_STATUS, 'VISION+NI', '2', $GetCatalogsEquivalenceResponse) }</AZXNAI-M-MARITAL-STATUS>    	
		            )else(
                ...
                let $LEGAL_ID_OTHER := string($IDENTIFICATION_ITEM[fn:upper-case(ns1:DocumentName) = "LICENCIA" or fn:upper-case(ns1:DocumentName) = "RTN"]/ns1:LegalId/text())
	            ...
	            let $COLONY_DESCRIPTION := string($ADDRESS_ITEM[ns1:Type = "1"]/ns1:Domicile/text())
                    let $COLONY_DESCRIPTION2 := string($ADDRESS_ITEM[ns1:Type = "2"]/ns1:Domicile/text())
	            ...
	            let $DEPARMENT_CODE := fn:upper-case(string($ADDRESS_ITEM[ns1:Type = "1"]/ns1:StateCode/text()))
	            let $MUNICIPALITY_CODE := fn:upper-case(string($ADDRESS_ITEM[ns1:Type = "1"]/ns1:CityCode/text()))
                    let $ZIP_CODE := string($ADDRESS_ITEM[ns1:Type = "1"]/ns1:ZipCode/text())
	            let $COUNTRY_CODE := fn:upper-case(string($ADDRESS_ITEM[ns1:Type = "1"]/ns1:CountryCode/text()))
	            let $PHONE_NUMBER := string($CONTACT_INFO[fn:upper-case(ns1:Type) = "LANDLINE"]/ns1:Number/text())
                    let $PHONE_WORK := string($CONTACT_INFO[fn:upper-case(ns1:Type) = "REFERENCE" and ns1:Location="2"]/ns1:Number/text())
	          	let $PHONE_MOBILE := string($CONTACT_INFO[fn:upper-case(ns1:Type) = "MOBILE"]/ns1:Number/text())
                        let $CEDULA := string($IDENTIFICATION_ITEM[fn:upper-case(ns1:DocumentName) = "CEDULA" or fn:upper-case(ns1:DocumentName) = "PASAPORTE"]/ns1:LegalId/text())
	          	...
	          	let $MUNICIPALITY_CODE_WORK := fn:upper-case(string($ADDRESS_ITEM[ns1:Type = "2"]/ns1:CityCode/text()))
	          	let $COUNTRY_CODE_WORK := fn:upper-case(string($ADDRESS_ITEM[ns1:Type = "2"]/ns1:CountryCode/text()))
	          	...
	          	let $DEPARMENT_CODE_OTHER := fn:upper-case(string($ADDRESS_ITEM[ns1:Type = "3"]/ns1:StateCode/text()))
	          	let $MUNICIPALITY_CODE_OTHER := fn:upper-case(string($ADDRESS_ITEM[ns1:Type = "3"]/ns1:CityCode/text()))
	          	let $COUNTRY_CODE_OTHER := fn:upper-case(string($ADDRESS_ITEM[ns1:Type = "3"]/ns1:CountryCode/text()))
            	   ...
            	   if($COLONY_DESCRIPTION  != "")then(
					...
					return(
	           		...
	           		if($MUNICIPALITY_CODE != "")then(
		            		<AZXNAI-M-CITY>{ local:CatalogValueOSBToAPP($MUNICIPALITY_CODE, 'VISION+NI', '7', $GetCatalogsEquivalenceResponse) }</AZXNAI-M-CITY>
		           	)else(
		           			<AZXNAI-M-CITY/>	
		           	),
                                  <AZXNAI-M-STATE/>,
                                if($MUNICIPALITY_CODE != "")then(
	            	 	<AZXNAI-M-PSTL-CD>{ local:CatalogValueOSBToAPP($MUNICIPALITY_CODE, 'VISION+NI', '7', $GetCatalogsEquivalenceResponse) }</AZXNAI-M-PSTL-CD>
	           		)else(
	           			<AZXNAI-M-PSTL-CD/>
	           		),
	           	    if($COUNTRY_CODE != "")then(
		            		 <AZXNAI-M-CNTRY-CD>{ local:CatalogValueOSBToAPP($COUNTRY_CODE, 'VISION+NI', '9', $GetCatalogsEquivalenceResponse) }</AZXNAI-M-CNTRY-CD>
	           		)else(
	           			<AZXNAI-M-CNTRY-CD/>
	           		) ,
		            ...
		            return(
			            if($GENDER  != "")then(
			            	<AZXNAI-M-GENDER-CODE>{ local:CatalogValueOSBToAPP($GENDER, 'VISION+NI', '1', $GetCatalogsEquivalenceResponse) }</AZXNAI-M-GENDER-CODE>
			            )else(
					...
					let $LEGAL_ID_TYPE:= fn:upper-case(string($IDENTIFICATION_ITEM[1]/ns1:DocumentName/text()))
					return(
						<AZXNAI-M-SSAN-FLAG>{ local:CatalogValueOSBToAPP($LEGAL_ID_TYPE, 'VISION+NI', '4', $GetCatalogsEquivalenceResponse) }</AZXNAI-M-SSAN-FLAG>,
					...
					return(
					 ...
					 return(
						if($PROFESSION_CODE  != "")then(
							<AZXNAI-M-POSITION>{ local:CatalogValueOSBToAPP($PROFESSION_CODE, 'VISION+NI', '3', $GetCatalogsEquivalenceResponse) }</AZXNAI-M-POSITION>
						)else(
							<AZXNAI-M-POSITION/>           	
						)
					 ),
					...
					return(
						if($JOB_TITLE != "")then(
							<AZXNAI-M-USER-3>{ local:CatalogValueOSBToAPP($JOB_TITLE, 'VISION+NI', '11', $GetCatalogsEquivalenceResponse) }</AZXNAI-M-USER-3>
						)else(
							<AZXNAI-M-USER-3/>	            	
						)
					),
                                        let $LineOfBusiness := fn:string($CreateCustomerRequest/ns1:CustomerInfo/ns1:LineOfBusiness/text())
                                        return(
                                          if ($LineOfBusiness != "") then (
                                            <AZXNAI-M-USER-4>{ $LineOfBusiness }</AZXNAI-M-USER-4>
                                          ) else (<AZXNAI-M-USER-4/>)
                                        ),
                                        let $IncomeRange := fn:string($CreateCustomerRequest/ns1:CustomerInfo/ns1:IncomeSourceList[ns1:CurrentlyEmployed = 'Yes']/ns1:IncomeRange/text())
                                        return (
                                          if ($IncomeRange != "") then (
                                            <AZXNAI-M-USER-5>{ $IncomeRange }</AZXNAI-M-USER-5>
                                          ) else (<AZXNAI-M-USER-5/>)
                                        ),
					 <AZXNAI-M-USER-6/>,
					 <AZXNAI-M-USER-7/>,
					...
					return(
						if($EXPIRATION_DATE != "")then(
							...
							<AZXNAI-M-USER-10>0000000</AZXNAI-M-USER-10>	            	
						)
					),
					...
					return(
						if(string($ISSUE_DATE/text())  != "")then(
							...
							<AZXNAI-M-USER-11>0000000</AZXNAI-M-USER-11>	            	
						)
					),
                                        let $COUNTRY_CODE_ISSUANCE := fn:upper-case(string($CreateCustomerRequest/ns1:CustomerInfo/ns1:Nationality/text()))
					return(
						if($COUNTRY_CODE_ISSUANCE != "")then(
							<AZXNAI-M-USER-12>{ local:CatalogValueOSBToAPP($COUNTRY_CODE_ISSUANCE, 'VISION+NI', '9', $GetCatalogsEquivalenceResponse) }</AZXNAI-M-USER-12>
						)else(
							<AZXNAI-M-USER-12/>	            	
						)
					),
                                        let $BirthCity := fn:string($CreateCustomerRequest/ns1:CustomerInfo/ns1:BirthCity/text())
                                        return (
                                          if($BirthCity != "" ) then (
                                            <AZXNAI-M-USER-13>{ $BirthCity }</AZXNAI-M-USER-13>
                                          ) else (<AZXNAI-M-USER-13/>)
                                        ),
					...
					return(
						if($LEVEL_OF_EDUCATION != "")then(
							<AZXNAI-M-USER-14>{ local:CatalogValueOSBToAPP($LEVEL_OF_EDUCATION, 'VISION+NI', '10', $GetCatalogsEquivalenceResponse) }</AZXNAI-M-USER-14>
						)else(
							<AZXNAI-M-USER-14/>	            	
						)
					),
					let $INCOMES := string($CreateCustomerRequest/ns1:CustomerInfo/ns1:TotalIncome/text())
					return(
						if($INCOMES != "")then(
					 ...
					 let $OTHERS_INCOMES := string($CreateCustomerRequest/ns1:CustomerInfo/ns1:IncomeSourceList[ns1:CurrentlyEmployed = 'Yes']/ns1:OtherIncomeSourceList/ns1:Amount/text())
					return(
						if($OTHERS_INCOMES != "")then(
							...
							<AZXNAI-M-USER-DEMO-1/>	            	
                                        ...
                                        if($COUNTRY_CODE != "")then(
							<AZXNAI-M-USER-DEMO-2>{ local:CatalogValueOSBToAPP($COUNTRY_CODE, 'VISION+NI', '9', $GetCatalogsEquivalenceResponse) }</AZXNAI-M-USER-DEMO-2>
						)else(
					...
					<AZXNAI-M-SUFFIX/>,
					<AZXNAI-M-COUNTY/>,(: Pendiente :)
					let $FREQUENT_FLYEER := string($CreateCustomerRequest/ns1:CustomerInfo/ns1:FrequentFlyeer/text())
					return(
						if($FREQUENT_FLYEER != "")then (
					...
					if($DEPARMENT_CODE_WORK != "")then(
		            		<AZXNAI-M-CITY-2>{ local:CatalogValueOSBToAPP($DEPARMENT_CODE_WORK, 'VISION+NI', '7', $GetCatalogsEquivalenceResponse) }</AZXNAI-M-CITY-2>
		           		)else(
		           			<AZXNAI-M-CITY-2/>  
		           	),
			            	if($MUNICIPALITY_CODE_WORK != "")then(
		            		<AZXNAI-M-PSTL-CD-2>{ local:CatalogValueOSBToAPP($MUNICIPALITY_CODE_WORK, 'VISION+NI', '7', $GetCatalogsEquivalenceResponse)}</AZXNAI-M-PSTL-CD-2>
			            )else(
			            	<AZXNAI-M-PSTL-CD-2/>  	            	
			        ),
			        if($COUNTRY_CODE_WORK != "")then(
		            	<AZXNAI-M-CNTRY-CD-2>{ local:CatalogValueOSBToAPP($COUNTRY_CODE_WORK, 'VISION+NI', '9', $GetCatalogsEquivalenceResponse) }</AZXNAI-M-CNTRY-CD-2>
			        )else(
			            <AZXNAI-M-CNTRY-CD-2/>  	            	
			        ),
			        ...
			        if($DEPARMENT_CODE_OTHER != "")then(	
		            		<AZXNAI-M-CITY-3>{ local:CatalogValueOSBToAPP($DEPARMENT_CODE_OTHER, 'VISION+NI', '7', $GetCatalogsEquivalenceResponse) }</AZXNAI-M-CITY-3>
			        )else(
			            	<AZXNAI-M-CITY-3/>         	
			        ),
			            	if($MUNICIPALITY_CODE_OTHER != "")then(
		            		<AZXNAI-M-PSTL-CD-3>{ local:CatalogValueOSBToAPP($MUNICIPALITY_CODE_OTHER, 'VISION+NI', '7', $GetCatalogsEquivalenceResponse) }</AZXNAI-M-PSTL-CD-3>
			        )else(
			            	<AZXNAI-M-PSTL-CD-3/>         	
			        ),
			        if($COUNTRY_CODE_OTHER != "")then(
		            		<AZXNAI-M-CNTRY-CD-3>{ local:CatalogValueOSBToAPP($COUNTRY_CODE_OTHER, 'VISION+NI', '9', $GetCatalogsEquivalenceResponse) }</AZXNAI-M-CNTRY-CD-3>
			            )else(
			            	<AZXNAI-M-CNTRY-CD-3>{ local:CatalogValueOSBToAPP($COUNTRY_CODE, 'VISION+NI', '9', $GetCatalogsEquivalenceResponse) }</AZXNAI-M-CNTRY-CD-3>         	
			         ),
		            ...
		            <AZXNAI-C-HOUSE-NBR/>,
                              if($COLONY_DESCRIPTION2  != "")then(
                                  <AZXNAI-C-HOUSE-NAME>{ data($COLONY_DESCRIPTION2) }</AZXNAI-C-HOUSE-NAME>
                              )else(
                                  <AZXNAI-C-HOUSE-NAME/>	            	
                              ),
		            <AZXNAI-C-ADDR-1/>,

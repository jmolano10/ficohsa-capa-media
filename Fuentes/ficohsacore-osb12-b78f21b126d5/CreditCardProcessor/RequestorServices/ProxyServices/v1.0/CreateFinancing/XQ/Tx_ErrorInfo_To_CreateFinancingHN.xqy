...
declare namespace ns1="http://www.ficohsa.com.hn/middleware.services/createFinancingTypes";
...
declare function local:func($errorInfo as element() (:: schema-element(ns2:ErrorInfo) ::)) as element() (:: schema-element(ns1:createFinancingResponse) ::) {
    <ns1:createFinancingResponse>
        <ErrorInfo>
            {
                ...
                <ns2:Details>
                    {
                        ...
                        then <ns2:SystemId>{fn:data($Details/ns2:SystemId)}</ns2:SystemId>
                        else ()
                    }
                    {
                        if ($Details/ns2:SystemStatus)
                        then <ns2:SystemStatus>{fn:data($Details/ns2:SystemStatus)}</ns2:SystemStatus>
                        else ()
                    }
                    {
                        if ($Details/ns2:MessageId)
                        then <ns2:MessageId>{fn:data($Details/ns2:MessageId)}</ns2:MessageId>
                        else ()
                    }
                    {
                        if ($Details/ns2:Messages)
                        then <ns2:Messages>{fn:data($Details/ns2:Messages)}</ns2:Messages>
                        else ()
                    }
                </ns2:Details>
            }

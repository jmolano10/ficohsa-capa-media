...
declare namespace xf = "http://tempuri.org/Middleware/v2/Resources/CambioFactorSeguridad/xq/usersValidationCustomServiceHeaderIn/";
...
declare function xq:getAction($operation as xs:string) as xs:string {
	if ($operation = "ValidateUserByUserName") then
		"urn:tailored.icbanking.services.administration.users.UsersValidationCustomService.ValidateUserByUserName"
	else if ($operation = "UpdateSecurityDeviceByUserName") then
		"urn:tailored.icbanking.services.administration.users.UsersValidationCustomService.UpdateSecurityDeviceByUserName"
	else 
		"Invalid Operation"
};

declare function xf:usersValidationCustomServiceHeaderIn($serviceAccount as xs:string,
    ...
    as element(*) {
        <soap-env:Header>
			<wsse:Security soap-env:mustUnderstand="true">
				<wsse:UsernameToken>
					<wsse:Username>{ 
						fn-bea:fail-over( xq:getUsername($serviceAccount), $serviceAccount) }
					</wsse:Username>
					<wsse:Password Type="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-username-token-profile-1.0#PasswordText">
						{ fn-bea:fail-over( xq:getPassword($serviceAccount), $serviceAccount) }
					</wsse:Password>
				</wsse:UsernameToken>
			</wsse:Security>
			 <urn:Context>
         ...
         <urn:UserTimeZoneName>SA Pacific Standard Time</urn:UserTimeZoneName>
			...
			<wsa:To soap-env:mustUnderstand="1">https://hnbnkpqawicbkap.gfficohsa.hn/AVATAR_NC.Tailored.ICBanking.ServiceHost/Administration/UsersValidationCustomService.svc</wsa:To>
			<wsa:Action>{ xq:getAction($operation) }</wsa:Action>
		</soap-env:Header>
};
...
xf:usersValidationCustomServiceHeaderIn($serviceAccount,
    $operation)
...
declare namespace xf = "http://tempuri.org/Middleware/v2/BusinessServices/ICB/CreaUsuarioSistema/biz/creaUsuarioSistemaHeaderIN/";
...
declare function xq:getAction($operation as xs:string) as xs:string {
	if ($operation = "GetUserByDocumentNumber") then
		"urn:tailored.icbanking.services.administration.users.GetUserByDocumentNumber"
	else 
		"Invalid Operation"
};
declare function xf:creaUsuarioSistemaHeaderIN($serviceAccount as xs:string,
    $operation as xs:string,$uriICB as xs:string )
    as element(*) {
        <soap-env:Header>
...
xf:creaUsuarioSistemaHeaderIN($serviceAccount,
    $operation,$uriICB )
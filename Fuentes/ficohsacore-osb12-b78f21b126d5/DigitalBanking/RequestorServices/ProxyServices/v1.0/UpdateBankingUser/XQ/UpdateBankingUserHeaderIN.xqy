...
declare namespace ns1="http://www.ficohsa.com/midleware.services/CreateBankingUserHeaderIN";
...
declare variable $uriICB as xs:string external;

(: Función encargada de realizar el mapeo de usuario:)
declare function xq:getUsername ($LDAPUsername as xs:string)  as xs:string { 
	let $data := fn-bea:lookupBasicCredentials(concat("DigitalBanking/RequestorServices/ProxyServices/v1.0/CreateBankingUser/SA/CreateBankingUser/", upper-case($LDAPUsername)))
  		return
...
declare function xq:getAction($operation as xs:string) as xs:string {
	if ($operation = "SynchronizeNewSiteProductsOnDemand") then
		"urn:tailored.icbanking.services.administration.sites.SitesCustomService.SynchronizeNewSiteProductsOnDemand"
	else 
		"Invalid Operation"
};

declare function ns1:createBankingUserHeaderIN($serviceAccount as xs:string, 
                                               $operation as xs:string, 
                                               $uriICB as xs:string) as element(*){
    <soap-env:Header>
            ...
            <wsa:To soap-env:mustUnderstand="1">{ $uriICB }</wsa:To>
            <wsa:Action>{ xq:getAction($operation) }</wsa:Action>
    </soap-env:Header>
};

ns1:createBankingUserHeaderIN($serviceAccount, $operation, $uriICB)
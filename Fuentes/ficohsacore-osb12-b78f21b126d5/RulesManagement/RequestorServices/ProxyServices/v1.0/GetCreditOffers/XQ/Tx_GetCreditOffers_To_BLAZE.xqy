...
(:: import schema at "../../../../../ProviderServices/WSDL/GetCustomerOffers/RuleServiceDefinitionORPTMOHNService.wsdl" ::)
...
declare function xq:Tx_GetCreditOffers_To_BLAZE($getCreditOffersRequest as element() (:: schema-element(ns1:getCreditOffersRequest) ::)) as element() (:: schema-element(ns2:epOriginacionesHonduras) ::) {
    <ns2:epOriginacionesHonduras>
        <arg0>
            ...
            <fechaNacimiento>{ data($getCreditOffersRequest/ns1:CustomerInfo/ns1:DateOfBirth) }</fechaNacimiento>
                ...
                {
                    for $OFFER in $getCreditOffersRequest/ns1:Offers/ns1:Offer
                    return
                    <mantenimientoPrestamo>
                        <codResolucionRetanqueoMC>{ data($OFFER/ns1:Resolution/ns1:Code) }</codResolucionRetanqueoMC>
                        <cuotaRetanqueoMC>{ data($OFFER/ns1:Installment) }</cuotaRetanqueoMC>
                        {
                          for $FLAG_HIT in $getCreditOffersRequest/ns1:CustomerInfo/ns1:FlagHit
                          return
                              <flagHit>{ data($FLAG_HIT) }</flagHit>
                        }
                        {
                          let $AMOUNT_CALCULATIONS := $OFFER/ns1:AmountCalculations
                          return(
                            <monto12RetanqueoMC>{ data($AMOUNT_CALCULATIONS/ns1:For12Months) }</monto12RetanqueoMC>,
                            <monto18RetanqueoMC>{ data($AMOUNT_CALCULATIONS/ns1:For18Months) }</monto18RetanqueoMC>,
                            <monto24RetanqueoMC>{ data($AMOUNT_CALCULATIONS/ns1:For24Months) }</monto24RetanqueoMC>,
                            <monto36RetanqueoMC>{ data($AMOUNT_CALCULATIONS/ns1:For36Months) }</monto36RetanqueoMC>,
                            <monto48RetanqueoMC>{ data($AMOUNT_CALCULATIONS/ns1:For48Months) }</monto48RetanqueoMC>,
                            <monto60RetanqueoMC>{ data($AMOUNT_CALCULATIONS/ns1:For60Months) }</monto60RetanqueoMC>
                            )
                        }
                        <montoRetanqueoMC>{ data($OFFER/ns1:MaxAmount) }</montoRetanqueoMC>
                        <tasaRetanqueoMC>{ data($OFFER/ns1:Rate) }</tasaRetanqueoMC>
                        <timesIncomeRetanqueoMC>{ data($OFFER/ns1:TimesIncome) }</timesIncomeRetanqueoMC>
                    </mantenimientoPrestamo>
                }
      {
...
xq:Tx_GetCreditOffers_To_BLAZE($getCreditOffersRequest)
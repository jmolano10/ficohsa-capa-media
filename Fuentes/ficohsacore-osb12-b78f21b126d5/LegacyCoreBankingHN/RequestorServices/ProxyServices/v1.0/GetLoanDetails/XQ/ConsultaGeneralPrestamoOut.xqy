...
(:: import schema at "../../../../../ProviderServices/XSD/GetLoanDetails/consultaGeneralPrestamo_db.xsd" ::)
...
declare function ns2:consultaGeneralPrestamoOut($ConsultaGeneralPrestamoRSP as element() (:: schema-element(ns1:OutputParameters) ::), $uuid as xs:string) as element() (:: schema-element(ns3:getLoanDetailsResponse) ::) {
    <ns3:getLoanDetailsResponse>
        ...
        <ns3:LoanDetails>
            ...
            {
                if ($ConsultaGeneralPrestamoRSP/ns1:NUMBER_OF_PAYMENTS)
                then <ns3:NumberPayments>{fn:data($ConsultaGeneralPrestamoRSP/ns1:NUMBER_OF_PAYMENTS)}</ns3:NumberPayments>
                else ()
            }
            {
                if ($ConsultaGeneralPrestamoRSP/ns1:PAID_PAYMENTS)
                then <ns3:PaidPayments>{fn:data($ConsultaGeneralPrestamoRSP/ns1:PAID_PAYMENTS)}</ns3:PaidPayments>
                else ()
            }
            {
            ...
            {
                if ($ConsultaGeneralPrestamoRSP/ns1:LOAN_STATUS)
                then <ns3:LoanStatus>{fn:data($ConsultaGeneralPrestamoRSP/ns1:LOAN_STATUS)}</ns3:LoanStatus>
                else ()
            }
            {
                if ($ConsultaGeneralPrestamoRSP/ns1:PRODUCT_CODE)
                then <ns3:ProductCode>{fn:data($ConsultaGeneralPrestamoRSP/ns1:PRODUCT_CODE)}</ns3:ProductCode>
                else ()
            }
            {
                if ($ConsultaGeneralPrestamoRSP/ns1:PRODUCT_DESC)
                then <ns3:ProductDescription>{fn:data($ConsultaGeneralPrestamoRSP/ns1:PRODUCT_DESC)}</ns3:ProductDescription>
                else ()
            }
            {
                if ($ConsultaGeneralPrestamoRSP/ns1:TYPE_GUARANTEE)
                then <ns3:GuaranteeTypeCode>{fn:data($ConsultaGeneralPrestamoRSP/ns1:TYPE_GUARANTEE)}</ns3:GuaranteeTypeCode>
                else ()
            }
            {
                if ($ConsultaGeneralPrestamoRSP/ns1:GUARANTEE_DESC)
                then <ns3:GuaranteeDescription>{fn:data($ConsultaGeneralPrestamoRSP/ns1:GUARANTEE_DESC)}</ns3:GuaranteeDescription>
                else ()
            }
            {
                if ($ConsultaGeneralPrestamoRSP/ns1:CREDIT_TYPE_CODE)
                then <ns3:CreditTypeCode>{fn:data($ConsultaGeneralPrestamoRSP/ns1:CREDIT_TYPE_CODE)}</ns3:CreditTypeCode>
                else ()
            }
            {
                if ($ConsultaGeneralPrestamoRSP/ns1:CREDIT_TYPE_DESC)
                then <ns3:CreditTypeDescription>{fn:data($ConsultaGeneralPrestamoRSP/ns1:CREDIT_TYPE_DESC)}</ns3:CreditTypeDescription>
                else ()
            }
        </ns3:LoanDetails>
    </ns3:getLoanDetailsResponse>

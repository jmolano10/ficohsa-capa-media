...
declare function xq:autorizaFtRemIn($usuarioAut as xs:string, 
    ...
    <ns1:AutorizacionreversionFT>
        <WebRequestCommon>
            <userName>{ fn-bea:fail-over( xq:getUsername(data($usuarioAut)),data($usuarioAut)) }</userName>
            <password>{ fn-bea:fail-over( xq:getPassword(data($usuarioAut)),data($passAut)) }</password>
        </WebRequestCommon>

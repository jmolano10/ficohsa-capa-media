...
declare function local:func($username as xs:string, 
    ...
    <ns5:InputParameters>
          <ns3:CODIGO_CONTRATO>{ data($paymentCollectionrequest/ns1:ContractId) }</ns3:CODIGO_CONTRATO>
            ...
            <ns3:INFO_T24>
                {
                    for $nodo in (local:get-nodes($getCollectionContractDetailResponse), local:get-nodes($tellerType), local:get-nodes($status), local:get-nodes($getCoreParametersResponse))
                    return
                    	<ns3:DATA>
		                    <ns3:NAME>{ fn:local-name($nodo) }</ns3:NAME>
		                    <ns3:VALUE>{ fn:string($nodo/text()) }</ns3:VALUE>
...
local:func($username, $password, $status, $tellerType, $paymentCollectionrequest, $getCollectionContractDetailResponse, $getCoreParametersResponse, $getRelationOperationCollectionResponse)
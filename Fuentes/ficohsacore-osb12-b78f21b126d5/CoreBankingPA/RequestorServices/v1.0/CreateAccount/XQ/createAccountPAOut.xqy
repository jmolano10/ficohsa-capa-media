...
declare namespace ns1="http://xmlns.oracle.com/pcbpel/adapter/db/sp/CreateAccount_BS";
...
declare variable $parameter2 as xs:string external;
...
declare function xq:createAccountGTOut($outputParameters as element() (:: schema-element(ns1:OutputParameters) ::), 
    ...
    <ns2:createAccountResponse>
        ...
        {
            if ($outputParameters/ns1:NUMERO_DE_CUENTA)
            then <ns2:AccountNumber>{fn:data($outputParameters/ns1:NUMERO_DE_CUENTA)}</ns2:AccountNumber>
            else ()
        }
    </ns2:createAccountResponse>
};

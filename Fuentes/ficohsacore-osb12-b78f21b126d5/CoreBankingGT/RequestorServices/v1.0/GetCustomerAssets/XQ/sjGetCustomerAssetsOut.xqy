...
declare namespace ns2="http://xmlns.oracle.com/pcbpel/adapter/db/sp/GetAccountsList_BS";
(:: import schema at "../../../../ProviderServices/XSD/GetAccountsList/GetAccountsList_sp.xsd" ::)
declare namespace ns3="http://xmlns.oracle.com/pcbpel/adapter/db/sp/GetTermDepositsList_BS";
    ...
    <ns4:OutputParameters>
        <ns4:OutputParametersDepositos>
            ...
            <ns4:PV_ASSET_STATUS>
            {
                for $ASSET_NUMBER_ITEM6 in $getTermDepositsListRSP/ns3:ASSET_AVAILABLE_BALANCE/ns3:ASSET_NUMBER_ITEM
                return 
                <ns4:PV_ASSET_NAME_ITEM>ACTIVE</ns4:PV_ASSET_NAME_ITEM>
            }
            </ns4:PV_ASSET_STATUS>
            {
                if ($getTermDepositsListRSP/ns3:ASSET_OPENING_DATE)
                then <ns4:PD_ASSET_OPENING_DATE>
                    {
                        for $ASSET_OPENING_DATE_ITEM in $getTermDepositsListRSP/ns3:ASSET_OPENING_DATE/ns3:ASSET_OPENING_DATE_ITEM
                        return 
                        <ns4:ASSET_OPENING_DATE_ITEM>{ fn-bea:dateTime-to-string-with-format("yyyy-MM-dd", fn:data($ASSET_OPENING_DATE_ITEM)) }</ns4:ASSET_OPENING_DATE_ITEM>
                    }</ns4:PD_ASSET_OPENING_DATE>
                else ()
            }
        ...
        <ns4:OutputParametersCuentas>
            ...
            {
                if ($getAccountsListRSP/ns2:PV_ASSET_STATUS)
                then <ns4:PV_ASSET_STATUS>
                    {
                        for $PV_ASSET_NAME_ITEM2 in $getAccountsListRSP/ns2:PV_ASSET_STATUS/ns2:PV_ASSET_NAME_ITEM
                        return 
                        <ns4:PV_ASSET_NAME_ITEM>{fn:data($PV_ASSET_NAME_ITEM2)}</ns4:PV_ASSET_NAME_ITEM>
                    }</ns4:PV_ASSET_STATUS>
                else ()
            }
            {
                if ($getAccountsListRSP/ns2:PD_ASSET_OPENING_DATE)
                then <ns4:PD_ASSET_OPENING_DATE>
                    {
                        for $ASSET_OPENING_DATE_ITEM in $getAccountsListRSP/ns2:PD_ASSET_OPENING_DATE/ns2:PD_ASSET_OPENING_DATE_ITEM
                        return 
                        <ns4:ASSET_OPENING_DATE_ITEM>{ fn-bea:dateTime-to-string-with-format("yyyy-MM-dd", fn:data($ASSET_OPENING_DATE_ITEM)) }</ns4:ASSET_OPENING_DATE_ITEM>
                    }</ns4:PD_ASSET_OPENING_DATE>
                else ()
            ...
            {
                if ($getAccountsListRSP/ns2:PV_ASSET_BLOCK)
                then <ns4:PV_ASSET_BLOCK>
                    {
                        for $PV_ASSET_NAME_ITEM3 in $getAccountsListRSP/ns2:PV_ASSET_BLOCK/ns2:PV_ASSET_NAME_ITEM
                        return 
                        <ns4:PV_ASSET_NAME_ITEM>{fn:data($PV_ASSET_NAME_ITEM3)}</ns4:PV_ASSET_NAME_ITEM>
                    }</ns4:PV_ASSET_BLOCK>
                else ()
            }
            {

    ...
    <ns3:getCustomerAssetsResponse>
        ...
        </ns3:StatusInfo>
        {
            if (fn:string($getAccountsListRSP/ns2:PV_ASSET_TYPE) = "AHO") then (
                ...
                let $assetStatus := $getAccountsListRSP/ns2:PV_ASSET_STATUS/ns2:PV_ASSET_NAME_ITEM
                let $assetBlock := $getAccountsListRSP/ns2:PV_ASSET_BLOCK/ns2:PV_ASSET_NAME_ITEM
                let $assetOpeningDate := $getAccountsListRSP/ns2:PD_ASSET_OPENING_DATE/ns2:PD_ASSET_OPENING_DATE_ITEM
                      ...
                      <ns3:Status>{ xq:mensajeRespuesta(data($getAccountsListRSP/ns2:PV_ERROR_CODE), $getAccountsListRSP/ns2:PV_ASSET_TYPE[text() = 'AHO']) }</ns3:Status>
                      {
                          ...
                          return
                              <ns3:AssetDetails>
                                  ...
                                  <ns3:SourceBank>GT01</ns3:SourceBank>
                                  <ns3:OperationType>{ data($assetProductType[$i]) }</ns3:OperationType>
                                  <ns3:Status>{if (data($assetStatus[$i]) = "INACTIVE") then (data($assetBlock[$i])) else (data($assetStatus[$i]))}</ns3:Status>
                                  <ns3:OpeningDate>{ fn-bea:dateTime-to-string-with-format("yyyy-MM-dd", data($assetOpeningDate[$i])) }</ns3:OpeningDate>
                              </ns3:AssetDetails>
                      }
                  </ns3:Assets>
                      ...
                      <ns3:Status>{ xq:mensajeRespuesta(data($getAccountsListRSP/ns2:PV_ERROR_CODE), $getAccountsListRSP/ns2:PV_ASSET_TYPE[text() = 'CHQ']) }</ns3:Status>
                      {
                          ...
                          return
                              <ns3:AssetDetails>
                                  ...
                                  <ns3:SourceBank>GT01</ns3:SourceBank>
                                  <ns3:OperationType>{ data($assetProductType[$i]) }</ns3:OperationType>
                                  <ns3:Status>{ data($assetStatus[$i])}</ns3:Status>
                                  <ns3:OpeningDate>{ fn-bea:dateTime-to-string-with-format("yyyy-MM-dd", data($assetOpeningDate[$i])) }</ns3:OpeningDate>
                              </ns3:AssetDetails>
                      }
                  </ns3:Assets>

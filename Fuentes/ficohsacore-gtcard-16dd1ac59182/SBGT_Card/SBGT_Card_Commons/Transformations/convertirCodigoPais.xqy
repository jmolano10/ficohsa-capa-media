...
declare function local:obtenerXMLPaises()
    as element(*){
    	<COUNTRY_CODES>
...
declare function local:convertirCodigoPais($codigoPais as xs:string,
    $sentidoConversion as xs:string)
    as xs:string {
		
			if(upper-case($sentidoConversion) = 'OSB-ISO3')then(		
					
				let $codEquivalente := string(local:obtenerXMLPaises()/CONVERSION_TYPE[ @tipo='ISO3' ]/COUNTRY_CODE[ @osb=$codigoPais ]/text())
				return
				...
				let $codEquivalente := string(local:obtenerXMLPaises()/CONVERSION_TYPE[ @tipo='ISO2' ]/COUNTRY_CODE[ @osb=$codigoPais ]/text())
				return
					if($codEquivalente != '')then(
						$codEquivalente
					)else(
				...
				let $codEquivalente := string(local:obtenerXMLPaises()/CONVERSION_TYPE[ @tipo='ISO2' ]/COUNTRY_CODE[ text()=$codigoPais ]/@osb)
				return
					if($codEquivalente != '')then(
						$codEquivalente
					)else(
				...
				let $codEquivalente := string(local:obtenerXMLPaises()/CONVERSION_TYPE[ @tipo='ISO3' ]/COUNTRY_CODE[ text()=$codigoPais ]/@osb)
				return
					if($codEquivalente != '')then(
						$codEquivalente
					)else(
...
local:convertirCodigoPais($codigoPais,
    $sentidoConversion)
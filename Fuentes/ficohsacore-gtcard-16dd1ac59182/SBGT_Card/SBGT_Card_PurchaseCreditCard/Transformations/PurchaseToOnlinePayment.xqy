...
declare function local:funcPurchaseToOnlinePayment($PurchaseCreditCard as element() (:: schema-element(ns1:purchaseCreditCard) ::),
    ...
    return
        ...
        <CardNumber>{fn:data($PurchaseCreditCard/CreditCard)}</CardNumber>
        <OrgId>{fn:data($org)}</OrgId>
        <MerchantNumber>{fn:data($PurchaseCreditCard/MerchantNumber)}</MerchantNumber>
        <CardExpirationDate>{fn:data($PurchaseCreditCard/CardExpirationDate)}</CardExpirationDate>
        <TotalSalesAmount>{fn:data($PurchaseCreditCard/PaymentAmount)}</TotalSalesAmount>
...
local:funcPurchaseToOnlinePayment($PurchaseCreditCard,$sequence,$org,$requestType)
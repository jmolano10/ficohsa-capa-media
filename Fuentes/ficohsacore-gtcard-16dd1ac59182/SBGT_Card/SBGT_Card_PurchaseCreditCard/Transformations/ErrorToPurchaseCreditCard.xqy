...
declare variable $ErrorToPurchaseCreditCard as element() (:: schema-element(ns1:errorMappingRegionalOutput) ::) external;
...
declare function local:funcErrorToPurchaseCreditCard($ErrorToPurchaseCreditCard as element() (:: schema-element(ns1:errorMappingRegionalOutput) ::),
                                                      $codeError as xs:string, 
                                                    $messageError as xs:string, 
                                                    $targetSystem as xs:string, 
                                                    $status as xs:string, 
                                                    $globalId as xs:string,
                                                    $messaggeOnlinePayment as xs:string) as element() (:: schema-element(ns2:purchaseCreditCardResponse) ::) {
    <ns2:purchaseCreditCardResponse>
      <StatusInfo>
        ...
        <ErrorInfo>
            <Code>{fn:data($ErrorToPurchaseCreditCard/ns1:code)}</Code>
            <Error>{fn:data($ErrorToPurchaseCreditCard/ns1:error)}</Error>
            <Description>{fn:data($ErrorToPurchaseCreditCard/ns1:description)}</Description>
            <ShortDescription>{fn:data($ErrorToPurchaseCreditCard/ns1:shortDescription)}</ShortDescription>
            <DateTime>{fn:substring(fn:string(fn:current-dateTime()),0,20)}</DateTime>
            <GlobalId>{fn:data($globalId)}</GlobalId>
            <Details>
...
local:funcErrorToPurchaseCreditCard($ErrorToPurchaseCreditCard, $codeError, $messageError, $targetSystem, $status, $globalId, $messaggeOnlinePayment)
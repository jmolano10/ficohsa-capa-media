...
declare namespace xf = "http://tempuri.org/MWHostToHost/v1/BusinessServices/ICB/TransferenciasInternacionales/xq/TranferH2HHeaderIN/";
...
declare function getAction($operation as xs:string) as xs:string {
	if ($operation = "ProcessExternalTransfer") then
		"urn:tailored.icbanking.services.transfersh2h.TransfersH2HCustomService.ProcessExternalTransfer"
	else 
		"Invalid Operation"
};
...
declare function xf:TranferH2HHeaderIN($ServiceAccount as xs:string,
    ...
    as element(*) {
     <soap-env:Header>
			...
			<urn:Context>
				...
				<urn:Channel>{$Channel}</urn:Channel>
		    	...
		    	<urn:SiteId>14332</urn:SiteId>
			    <urn:SiteTypeId>2</urn:SiteTypeId>
...
xf:TranferH2HHeaderIN($ServiceAccount,
    $Operation,
    $Channel,
    $uriICB)
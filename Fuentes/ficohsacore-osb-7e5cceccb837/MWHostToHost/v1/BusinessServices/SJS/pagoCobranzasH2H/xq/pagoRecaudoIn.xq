        ...
        <ns1:pagoRecaudo>
        {
            if ( count($PT_CAMPOSPORSERVICIO/ns2:PT_CAMPOSPORSERVICIO_ITEM) = 0 ) then (
            	...
            	<DEBTOR_CODE>{ data($DEBTOR/CODE) }</DEBTOR_CODE>,
           		 ...
           		 <BILL_INFORMATION>
                	<BILL_NUMBER>{ data($DEBTOR/BILL_INFORMATION/BILL_NUMBER) }</BILL_NUMBER>
                	<DUE_DATE>{ data($DEBTOR/BILL_INFORMATION/DUE_DATE) }</DUE_DATE>
           		 </BILL_INFORMATION>,
        		...
        		let $data := for $posicion in 1 to $maxPosicion
	        		...
	        		if ( string($errorData/text()) = '' )then (
            			...
            			<PAYMENT_INFORMATION>
                 		...
                 		<BILL_INFORMATION>
                			<BILL_NUMBER>{ data($DEBTOR/BILL_INFORMATION/BILL_NUMBER) }</BILL_NUMBER>
                			<DUE_DATE>{ fn:replace(fn:string(data($DEBTOR/BILL_INFORMATION/DUE_DATE)),'-','') }</DUE_DATE>
           		 		</BILL_INFORMATION>,
           				<EXTENDED_COLLECTION_YES_NO>{ data($DEBTOR/EXTENDED_COLLECTION) }</EXTENDED_COLLECTION_YES_NO>,  

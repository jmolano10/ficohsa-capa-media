...
(:: pragma bea:global-element-parameter parameter="$sjTransferenciaInternacionalResponse" element="ns2:sjTransferenciaInternacionalResponse" location="../../../BusinessServices/SJS/TransferenciaInternacionalH2H/xsd/sjTransferenciaInternacionalH2H.xsd" ::)
...
declare namespace ns2 = "http://www.ficohsa.com.hn/middleware.services/sjTransfersH2HTypes";
...
declare function xf:transferenciaInternacionalH2HICBIN($transferenciaInternacionalH2HRequest as element(ns1:transferenciaInternacionalH2HRequest),
    ...
    $sjTransferenciaInternacionalResponse as element(ns2:sjTransferenciaInternacionalResponse))
            ...
            <TRANSFERS_INT>
                {
			        ...
			        let $trace := data($transferInt/TRACE)
                    let $matchingResponse := $sjTransferenciaInternacionalResponse/TRANSFERS_INT/TRANSFER_INT[SUCCESS_INDICATOR = 'SUCCESS' and TRACE = $trace][1]
					where count($matchingResponse) > 0
                    return
                        <TRANSFER_INT>
                            ...
                            {
                            	let $valueDate := data($matchingResponse/VALUEDATE)
                            	let $date :=  fn-bea:date-to-string-with-format("yyyy-MM-dd", fn-bea:date-from-string-with-format('yyyyMMdd', $valueDate))
                              	return
                            ...
                            {
                            	let $refCode := data($matchingResponse/TRANSACTION_ID)
                              	return
...
declare variable $sjTransferenciaInternacionalResponse as element(ns2:sjTransferenciaInternacionalResponse) external;
...
xf:transferenciaInternacionalH2HICBIN($transferenciaInternacionalH2HRequest,
    ...
    $AMOUNTDEBIT,
    $CURRENCY,
    $sjTransferenciaInternacionalResponse)

...
declare namespace xf = "http://tempuri.org/MiddlewareCaja/Resources/xqueries/PagoDeCheque/pagoDeChequeLcyOut/";
declare namespace ns1 = "http://www.ficohsa.com.hn/middleware.services/pagoDeCheque";
declare namespace ns0 = "T24WebServicesImpl";

declare function xf:pagoDeChequeLcyOut($pagodeChequeLCYResponse1 as element(ns0:PagodeChequeLCYResponse),
    ...
    as element(ns1:pagoDeChequeResponse) {
        if (fn:string($pagodeChequeLCYResponse1/Status/successIndicator/text()) = "Success") then (
	        <ns1:pagoDeChequeResponse>
	            ...
	            {
	                for $CURRENCY2 in $pagodeChequeLCYResponse1/TELLERType/CURRENCY2
	                return
	                    <DEBIT_CURRENCY>{ data($CURRENCY2) }</DEBIT_CURRENCY>
	            }
	            {
	                for $AMOUNTLOCAL2 in $pagodeChequeLCYResponse1/TELLERType/AMOUNTLOCAL2
	                return
	                    <DEBIT_AMOUNT>{ data($AMOUNTLOCAL2) }</DEBIT_AMOUNT>
	            }
	            {
	                for $CHEQUENUMBER in $pagodeChequeLCYResponse1/TELLERType/gCHEQUENUMBER/mCHEQUENUMBER[1]/CHEQUENUMBER
	                return
	                    <CHEQUE_NUMBER>{ data($CHEQUENUMBER) }</CHEQUE_NUMBER>
	            }
	            {
	                let $result :=
	                    for $NARRATIVE2 in $pagodeChequeLCYResponse1/TELLERType/gNARRATIVE2/NARRATIVE2
	                    return
	            ...
	            {
	                let $result :=
	                    for $DATETIME in $pagodeChequeLCYResponse1/TELLERType/gDATETIME/DATETIME
	                    return
	            ...
	            {
	                let $INPUTTER := fn:tokenize(fn:string($pagodeChequeLCYResponse1/TELLERType/gINPUTTER/INPUTTER/text()),"_")
                    return
...
xf:pagoDeChequeLcyOut($pagodeChequeLCYResponse1,

...
(:: pragma bea:global-element-return element="ns0:InputParameters" location="../../xsds/RegistroBitacoraConvenio/RegistrarBitacoraResponseConvenio/FLINK_OSB_REG_BITACORA_RECAUDO_RSP.xsd" ::)

declare namespace ns1 = "http://www.ficohsa.com.hn/middleware.services/registroBitacoraConvenioTypes";
declare namespace ns0 = "http://xmlns.oracle.com/pcbpel/adapter/db/FLINK/OSB_REG_BITACORA_RECAUDO_RSP/";
declare namespace xf = "http://tempuri.org/MiddlewareCaja/Resources/xqueries/RegistroBitacoraConvenio/RegistrarBitacoraResponseConvenioIn/";

declare function xf:RegistrarBitacoraResponseConvenioIn($registroBitacoraConvenio as element(ns1:registroBitacoraConvenio))
    as element(ns0:InputParameters) {
        <ns0:InputParameters>
            ...
            {
                for $REQUEST_LOG_UUID in $registroBitacoraConvenio/REQUEST_LOG_UUID
                return
                    <ns0:P_REQ_UUID>{ data($REQUEST_LOG_UUID) }</ns0:P_REQ_UUID>
            }
            {
                for $ERROR_CODE in $registroBitacoraConvenio/ERROR_CODE
                return
                    <ns0:P_COD_ERROR_CORE>{ data($ERROR_CODE) }</ns0:P_COD_ERROR_CORE>
            }
            {
                for $ERROR_MESSAGE in $registroBitacoraConvenio/ERROR_MESSAGE
                return
                    <ns0:P_MSG_ERROR_CORE>{ data($ERROR_MESSAGE) }</ns0:P_MSG_ERROR_CORE>
            }
            {
                for $SOAP_MESSAGE in $registroBitacoraConvenio/SOAP_MESSAGE/*
                return
                    <ns0:P_RESPONSE>{ $SOAP_MESSAGE }</ns0:P_RESPONSE>
            }
        </ns0:InputParameters>
};
...
xf:RegistrarBitacoraResponseConvenioIn($registroBitacoraConvenio)
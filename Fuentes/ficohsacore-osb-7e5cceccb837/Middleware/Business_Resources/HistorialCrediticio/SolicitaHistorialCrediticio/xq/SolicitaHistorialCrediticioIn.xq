...
(:: pragma bea:global-element-return element="ext:eCartera" location="../webservicesirc.wsdl" ::)
...
declare function xf:SolicitaHistorialCrediticioIn($serviceAccount as xs:string,
    ...
    $legalDoc)
    as xs:string {
		fn:concat("25|", data($parametrizacionValue), "|", fn-bea:fail-over( getUsername($serviceAccount), $serviceAccount), "|", fn-bea:fail-over( getPassword($serviceAccount), $serviceAccount), "|", getNamesAndLastName($p1,1), "|",  getNamesAndLastName($p2,2), "|", data($p3), "|", getNamesAndLastName($p4,1), "|", getNamesAndLastName($p5,1), "|", data($p6), "|", data($p7), "|", getFixedNIT($p8), "|", getDPIParam( $legalDoc, $p9), "|", getParam( $legalDoc, $p10), "|", getParam( $legalDoc, $p11), "|", data($p12), "|", getPhoneNumber($p13)) 
};

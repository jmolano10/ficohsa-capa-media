...
(:: pragma bea:global-element-return element="FicCpa:consultaProductosAfiliadosResponse" location="../xsd/ConsultaProductosAfiliadosTypes.xsd" ::)
...
declare function xf:Tx_ConsultaProductosAfiliadosHeaderResp($getThirdPartyProductsForSaraResponse as element(ns8:GetThirdPartyProductsForSaraByDocumentNumberMessageOut))
    as element(auth:ResponseHeader) {
        <auth:ResponseHeader>
        	<successIndicator>{ calcSuccessIndicator(data($getThirdPartyProductsForSaraResponse/ns11:OperationResult/ns12:Value)) }</successIndicator>
        </auth:ResponseHeader>
};
...
xf:Tx_ConsultaProductosAfiliadosHeaderResp($getThirdPartyProductsForSaraResponse)
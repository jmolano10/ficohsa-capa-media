...
declare function xf:consultaTarjetasCreditoHNIn($consultaTarjetasCreditoPorCliente as element(ns0:consultaTarjetasCreditoPorCliente),$AplicaFiltros as xs:integer)
    as element(ns1:ConsultarTarjetas) {
        if( data($consultaTarjetasCreditoPorCliente/CUSTOMER_FILTER/CUSTOMER_ID_TYPE) = "LEGAL_ID" or $AplicaFiltros = 1 ) then (
...
declare variable $AplicaFiltros as xs:integer external;

xf:consultaTarjetasCreditoHNIn($consultaTarjetasCreditoPorCliente,$AplicaFiltros)
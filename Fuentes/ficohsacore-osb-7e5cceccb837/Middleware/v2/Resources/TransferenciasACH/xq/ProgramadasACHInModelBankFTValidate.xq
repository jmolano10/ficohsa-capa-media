    ...
    as element(ns2:Transfmodelbankentrecuentas) {
        <ns2:Transfmodelbankentrecuentas>
            ...
            <FUNDSTRANSFERRETAILINTSALEWSType id = ""> 
            	<TRANSACTIONTYPE>ACHC</TRANSACTIONTYPE> 
                ...
                <DEBITAMOUNT>{ xs:double(data($ACHRequest/TRANSFER_AMOUNT)) + xs:double($BankCharge) }</DEBITAMOUNT>
                ...
                <LRNARADDEND>{ substring(fn:string($ACHRequest/TRANSACTION_DESCRIPTION/text()),1,35)  }</LRNARADDEND> 
...
declare variable $BankCharge as xs:string external;

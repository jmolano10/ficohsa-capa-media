...
declare namespace xf = "http://tempuri.org/Middleware/v3/Resources/TransferenciasACH/xq/envioACHHNLIN.XQ/";

declare function xf:envioACHHNLIN($transferenciasACHRequest1 as element(ns1:transferenciasACHRequest),
    $FtTt as xs:string,
    ...
    as element(ns0:SendCreditHNL) {
        <ns0:SendCreditHNL>
            ...
            {
	            if (fn:string($transferenciasACHRequest1/ns0:ACCOUNT_TYPE/text()) = "Savings") then
	            (
	            	<ns0:vDestinationAccountType>2</ns0:vDestinationAccountType>
	            ) 
	            else if (fn:string($transferenciasACHRequest1/ns0:ACCOUNT_TYPE/text()) = "Checking") then
	            (   
	            	<ns0:vDestinationAccountType>1</ns0:vDestinationAccountType>
	            ) else if (fn:string($transferenciasACHRequest1/ns0:ACCOUNT_TYPE/text()) = "Loan") then
	            (
...
xf:envioACHHNLIN($transferenciasACHRequest1,
    $FtTt,
    $OrigAccountType,
    $receivingBankId,
    $originatorId,

                ...
                <FICOHNACHONLINEINPUTACHINType>
                    {
                        ...
                        return
                            <DEBITACCTNO>{ data($ACCOUNT) }</DEBITACCTNO>
                    }
                    ...
                    <TIPOTRANS>{ replace(data($transactionDetail/TRANSACTION_TYPE),'_','.') }</TIPOTRANS>
                    {
                        ...
                        return
                            <NARADDEND>{ fn:substring(data($DESCRIPTION), 1, 34) }</NARADDEND>
                    }
                    {
                        for $CUSTOMER_NAME in $transactionDetail/ORIGINATOR_INFO/CUSTOMER_NAME
                        return
                            <CUSNAME>{ fn:replace(fn:substring(data($CUSTOMER_NAME), 1, 34), '(\:|\[|\]|\\|\/|\||\"|\^|\,|\$|\?|\*|\+|\{|\}|\(|\))', '.') }</CUSNAME>
                    }

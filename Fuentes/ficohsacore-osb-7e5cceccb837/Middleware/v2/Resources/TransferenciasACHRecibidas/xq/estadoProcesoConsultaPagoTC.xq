...
declare namespace xf = "http://tempuri.org/Middleware/v2/Resources/TransferenciasACHRecibidas/xq/estadoProcesoConsultaPagoTC/";

declare function xf:estadoProcesoConsultaPagoTC($successIndicator as xs:string,
    	...
    	<VALIDACION_PROCESO>
    		{
				if($successIndicator = 'SUCCESS') then(
					...
					if($zeroRecords = '0 RECORDS RETURNED') then(
						...
						<VALIDACION_MENSAJE>T24-003A Registro no creado</VALIDACION_MENSAJE>, 
						<CODIGO_ERROR_PROCESO>T24-003A</CODIGO_ERROR_PROCESO>, 
						<MENSAJE_ERROR_PROCESO>Registro no creado</MENSAJE_ERROR_PROCESO>
						
					)else if($txnStatus = 'PENDING') then(
					
						<ESTADO_PROCESO>PENDING</ESTADO_PROCESO>, 
						<VALIDACION_MENSAJE>T24-003B Registro pendiente de finalizar</VALIDACION_MENSAJE>, 
						<CODIGO_ERROR_PROCESO>T24-003B</CODIGO_ERROR_PROCESO>, 
					...
					<ESTADO_PROCESO>PENDING</ESTADO_PROCESO>, 
					<VALIDACION_MENSAJE>{ fn:concat('TWS-E002 ', $messages) }</VALIDACION_MENSAJE>, 
					<CODIGO_ERROR_PROCESO>TWS-E002</CODIGO_ERROR_PROCESO>, 
					...
					<ESTADO_PROCESO>PENDING</ESTADO_PROCESO>, 
					<VALIDACION_MENSAJE>OSB-E005 Registro no creado</VALIDACION_MENSAJE>, 
...
xf:estadoProcesoConsultaPagoTC($successIndicator,
	$zeroRecords, 
	$messages,
	$txnStatus,
	$idRejection)
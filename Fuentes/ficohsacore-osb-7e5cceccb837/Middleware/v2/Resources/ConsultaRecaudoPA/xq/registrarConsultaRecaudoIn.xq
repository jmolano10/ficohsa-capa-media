(:: pragma  parameter="$responseXmlTelered" type="anyType" ::)
...
declare function xf:registrarConsultaRecaudoIn($responseXmlTelered as element(*),
    ...
    as element(ns0:InputParameters) {
        <ns0:InputParameters>
            ...
            <ns0:PC_RESPONSEXML>{ $responseXmlTelered }</ns0:PC_RESPONSEXML>
            <ns0:PV_IDCONVENIO>{ $contractId }</ns0:PV_IDCONVENIO>
            <ns0:PV_CODIGODEUDOR>{ $debtorCode }</ns0:PV_CODIGODEUDOR>
        </ns0:InputParameters>
};

declare variable $responseXmlTelered as element(*) external;
...
xf:registrarConsultaRecaudoIn($responseXmlTelered,
    $contractId,
    $debtorCode,
    $secuenciaConsulta)
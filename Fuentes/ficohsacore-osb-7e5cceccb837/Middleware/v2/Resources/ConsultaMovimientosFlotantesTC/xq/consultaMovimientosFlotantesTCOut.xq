...
declare function xf:consultaMovimientosFlotantesTCOut($outstandingAuthorizationInquiryResponse as element(ns1:OutstandingAuthorizationInquiryResponse),
    ...
    as element(ns0:consultaMovimientosFlotantesTCResponse) {
        <ns0:consultaMovimientosFlotantesTCResponse>
            <OWNER_NAME>{fn-bea:trim(fn:replace(fn:string(functx:replace-first($cardHolderName, '0', '-')),"-", " "))}</OWNER_NAME>
            <PRINCIPAL_CARD_NUMBER>{$carNumberRequest}</PRINCIPAL_CARD_NUMBER>
            <TOTAL_LCY_AMOUNT>{if ($lcyAmmount = '0') then '0.00' else fn-bea:format-number(number($lcyAmmount),"##0.00")}</TOTAL_LCY_AMOUNT>
            <TOTAL_USD_AMOUNT>{if ($usdAmmount = '0') then '0.00' else fn-bea:format-number(number($usdAmmount),"##0.00")}</TOTAL_USD_AMOUNT>
            <CARDS>
                <CARD_DETAILS>
                    <CARD_NUMBER>{ 
                     
                     let $cardNumber :=  fn-bea:trim(fn:replace(fn:string(functx:replace-first(data($lcymovements/OutstandingAuthArray/OutstandingAuthData[1]/OFXOAO-CARD-NBR), '0', '-')),"-", " "))
	                 return
		                if ($cardNumber  = '') then (
		              		 fn-bea:trim(fn:replace(fn:string(functx:replace-first(data($usdmovements/OutstandingAuthArray/OutstandingAuthData[1]/OFXOAO-CARD-NBR), '0', '-')),"-", " "))    
		               )
		               else
		               	$cardNumber   
                    
                    }</CARD_NUMBER>
                    <LCY_AMOUNT>{if ($lcyAmmount = '0') then '0.00' else fn-bea:format-number(number($lcyAmmount),"##0.00")}</LCY_AMOUNT>
                    <USD_AMOUNT>{if ($usdAmmount = '0') then '0.00' else fn-bea:format-number(number($usdAmmount),"##0.00")}</USD_AMOUNT>
                   <MOVEMENTS>
                        {
                            for $OutstandingAuthData in $usdmovements/OutstandingAuthArray/OutstandingAuthData
                            let $requestType := $OutstandingAuthData/OFXOAO-REQUEST-TYPE
                            let $respCode := $OutstandingAuthData/OFXOAO-B039-RESP-CODE
                            let $reasonDesc := fn:normalize-space($OutstandingAuthData/OFXOAO-REASON-DESC)
	                        return
	                            if ($requestType ='A' and $respCode ='00' and $reasonDesc='APPROVED') then (
	                                <FLOATING_MOVEMENT>

(:: pragma bea:global-element-parameter parameter="$outputParameters" element="ns1:OutputParameters" location="../../../BusinessServices/MasterData/conListadoPrestamos/xsd/conListadoPrestamos_sp.xsd" ::)
...
declare namespace xf = "http://tempuri.org/Middleware/v2/Resources/ConsultaFinanciamientosCliente/xq/consultaFinanciamientosClienteOut/";

declare function xf:consultaFinanciamientosClienteOut($outputParameters as element(ns1:OutputParameters))
    as element(ns0:consultaFinanciamientosClienteResponse) {
        <ns0:consultaFinanciamientosClienteResponse>
            {
            	for $rows in $outputParameters/ns1:RowSet/ns1:Row
            	return 
            	(
            	<consultaFinanciamientosClienteResponseRecordType>	
            	    {	
						let $producType := string($rows/ns1:Column[@name = 'ProductoFinanciamiento']/text())
						return
						<PRODUCT_TYPE>{$producType}</PRODUCT_TYPE>
					}
					{
...
xf:consultaFinanciamientosClienteOut($outputParameters)
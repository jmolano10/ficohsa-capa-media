...
declare function xf:Tx_RegistraPagoTC_To_osbConOrgSequence_db($outputParameters1 as element(ns0:OutputParameters),
    ...
    as element(ns3:OnlinePaymentV4Request) {
        <ns3:OnlinePaymentV4Request>
            ...
            <RRN37>
                {
                    ...
                    return
                        (concat($lastYearDigit ,$yearDays, $hours, $minutes, string($outputParameters2/ns2:SequenceValue/text())))
                }            
            ...
            {
	            if (fn:data($outputParameters1/ns0:CONFIGURACIONES/ns0:CONFIGURACIONES_ITEM[ns0:NAME='FICBCO0067.HN01.MCCTYPE']/ns0:VALUE)!='')
	            then
	            	...
	            	<MCCType>{ data($outputParameters1/ns0:CONFIGURACIONES/ns0:CONFIGURACIONES_ITEM[ns0:NAME='FICBCO0067.HN01.MCCTYPE.WITHDRAWAL']/ns0:VALUE) }</MCCType>         
            }
            <RequestType>{ data($outputParameters1/ns0:CONFIGURACIONES/ns0:CONFIGURACIONES_ITEM[ns0:NAME='FICBCO0067.HN01.AUTHORIZATION']/ns0:VALUE) }</RequestType>
            <CardNumber>{ data($registraPagoTCRequest1/CREDIT_CARD) }</CardNumber>
            <OrgId>{ data($outputParameters2/ns2:ORG) }</OrgId>
            {

...
declare function xf:registraSaldosNITCIn($sjConsultaSaldosTarjetaCreditoResponse as element(ns1:sjConsultaSaldosTarjetaCreditoResponse),
	         	...
	         	<ns2:PT_SALDOS_TARJETA_ITEM>
			        {
				        ...
				        let $currencyBase:= string($rowBase/ns0:Column[upper-case(@name) = 'CURRENCY']/text())
				        let $currencyAlt:= string($rowAlt/ns0:Column[upper-case(@name) = 'CURRENCY']/text())				        
	                	...
	                	if(not(empty(data($sjConsultaSaldosTarjetaCreditoResponse/LCY_MIN_PAYMENT/text()))))then(
	                		...
	                		if($currencyBase='NIO')then(
	                			...
	                			<ns2:LCY_MIN_PAYMENT>{ string($rowAlt/ns0:Column[@name = 'MIN_PAYMENT']/text())}</ns2:LCY_MIN_PAYMENT>	                		
	                		)
	                	),
	                	if(not(empty(data($sjConsultaSaldosTarjetaCreditoResponse/LCY_TOTAL_PAYMENT/text()))))then(
	                		if($currencyBase='NIO')then(
		                		if($LCY_TOTAL_PAYMENT_BASE >= 0)then(
		                			<ns2:LCY_TOTAL_PAYMENT>{ $LCY_TOTAL_PAYMENT_BASE }</ns2:LCY_TOTAL_PAYMENT>
		                		)else(
		                			<ns2:LCY_TOTAL_PAYMENT>0</ns2:LCY_TOTAL_PAYMENT>
		                		)	                			                		
	                		)else(
		                		if($LCY_TOTAL_PAYMENT_ALT >= 0)then(
		                			<ns2:LCY_TOTAL_PAYMENT>{ $LCY_TOTAL_PAYMENT_ALT }</ns2:LCY_TOTAL_PAYMENT>
		                		)else(
		                			<ns2:LCY_TOTAL_PAYMENT>0</ns2:LCY_TOTAL_PAYMENT>
		                		)	                		
	                		)
	                		...
	                		if($currencyBase='NIO')then(
	                			...
	                			<ns2:LCY_TOTAL_PAYMENT>{ string($rowAlt/ns0:Column[@name = 'TOTAL_PAYMENT']/text()) }</ns2:LCY_TOTAL_PAYMENT>	                		
	                		)
	                	),
	                	if(not(empty(data($sjConsultaSaldosTarjetaCreditoResponse/FCY_MIN_PAYMENT/text()))))then(
	                		...
	                		if($currencyBase='NIO')then(
	                			...
	                			<ns2:FCY_MIN_PAYMENT>{ string($rowBase/ns0:Column[@name = 'MIN_PAYMENT']/text())}</ns2:FCY_MIN_PAYMENT>	                		
	                		)
	                	),
	                	if(not(empty(data($sjConsultaSaldosTarjetaCreditoResponse/FCY_TOTAL_PAYMENT/text()))))then(
	                		if($currencyBase='NIO')then(
		                		if($FCY_TOTAL_PAYMENT_BASE >= 0)then(
		                			<ns2:FCY_TOTAL_PAYMENT>{ $FCY_TOTAL_PAYMENT_BASE }</ns2:FCY_TOTAL_PAYMENT>
		                		)else(
		                			<ns2:FCY_TOTAL_PAYMENT>0</ns2:FCY_TOTAL_PAYMENT>
		                		)	                		
	                		)else(
		                		if($FCY_TOTAL_PAYMENT_ALT >= 0)then(
		                			<ns2:FCY_TOTAL_PAYMENT>{ $FCY_TOTAL_PAYMENT_ALT }</ns2:FCY_TOTAL_PAYMENT>
		                		)else(
		                			<ns2:FCY_TOTAL_PAYMENT>0</ns2:FCY_TOTAL_PAYMENT>
		                		)	                		
	                		)
	                		...
	                		<ns2:FCY_TOTAL_PAYMENT>{ string($rowBase/ns0:Column[@name = 'TOTAL_PAYMENT']/text()) }</ns2:FCY_TOTAL_PAYMENT>	                		
	                		)
	                	...
	                	if(not(empty(data($sjConsultaSaldosTarjetaCreditoResponse/CREDIT_LIMIT_CCY/text()))))then(
	                		...
	                		if(($orgBase = '333') or ($orgBase = '332'))then(
		                		if(not(empty($rowBase))) then (
		                			<ns2:CREDIT_LIMIT_CCY>{ string($rowBase/ns0:Column[@name = 'CURRENCY']/text()) }</ns2:CREDIT_LIMIT_CCY>
		                		)else(
		                			<ns2:CREDIT_LIMIT_CCY>{ string($rowAlt/ns0:Column[@name = 'CURRENCY']/text()) }</ns2:CREDIT_LIMIT_CCY>
		                		)	                		
	                		)else(
		                		if(not(empty($rowAlt))) then (
		                			<ns2:CREDIT_LIMIT_CCY>{ string($rowAlt/ns0:Column[@name = 'CURRENCY']/text()) }</ns2:CREDIT_LIMIT_CCY>
...
xf:registraSaldosNITCIn($sjConsultaSaldosTarjetaCreditoResponse,

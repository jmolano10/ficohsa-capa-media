...
declare function xf:consultaSaldosTarjetaCreditoHNOut($consultaSaldosTarjetaCreditoResponse as element(ns1:ConsultaSaldosTarjetaCreditoResponse),
    ...
    as element(ns0:consultaSaldosTarjetaCreditoResponse) {
        <ns0:consultaSaldosTarjetaCreditoResponse>
           ...
           {
           		for $ConsultaSaldosTarjetaCreditoRecordDetail in $consultaSaldosTarjetaCreditoResponse/ns1:ConsultaSaldosTarjetaCreditoResult/ns2:ConsultaSaldosTarjetaCreditoResponse/ns2:ConsultaSaldosTarjetaCreditoRecordDetail
                return
		            <ns0:consultaSaldosTarjetaCreditoResponseType>
		            {
	            	  	for $ConsultaSaldosTarjetaCreditoRecordDetailDTO in $ConsultaSaldosTarjetaCreditoRecordDetail/ns2:ConsultaSaldosTarjetaCreditoRecordDetailDTO
                       	return
			                <ns0:consultaSaldosTarjetaCreditoResponseRecordType>
			                	{
			                		for $CURRENCY in $ConsultaSaldosTarjetaCreditoRecordDetailDTO/ns2:CURRENCY
                                    return
			                    		<CURRENCY>{ data($CURRENCY) }</CURRENCY>
			                    }
			                    {
			                    	for $OVERDUE_BALANCE in $ConsultaSaldosTarjetaCreditoRecordDetailDTO/ns2:OVERDUE_BALANCE
                                    return
			                    		<OVERDUE_BALANCE>{ data($OVERDUE_BALANCE) }</OVERDUE_BALANCE>
			                    }
			                    {
			                    	 for $FLOATING_BALANCE in $ConsultaSaldosTarjetaCreditoRecordDetailDTO/ns2:FLOATING_BALANCE
                                     return
			                    		<FLOATING_BALANCE>{ data($FLOATING_BALANCE) }</FLOATING_BALANCE>
			                    }
			                    {
			                    	for $CURRENT_BALANCE in $ConsultaSaldosTarjetaCreditoRecordDetailDTO/ns2:CURRENT_BALANCE
                                        return
			                  	    	<CURRENT_BALANCE>{ data($CURRENT_BALANCE) }</CURRENT_BALANCE>
			                    }
			                    {
			                    	for $PURCHASES_LIMIT in $ConsultaSaldosTarjetaCreditoRecordDetailDTO/ns2:PURCHASES_LIMIT
                                        return
			                    	<PURCHASES_LIMIT>{ data($PURCHASES_LIMIT) }</PURCHASES_LIMIT>
			                    }
			                    {
			                    	 for $WITHDRAWAL_LIMIT in $ConsultaSaldosTarjetaCreditoRecordDetailDTO/ns2:WITHDRAWAL_LIMIT
                                     return
			                    		<WITHDRAWAL_LIMIT>{ data($WITHDRAWAL_LIMIT) }</WITHDRAWAL_LIMIT>
			                    }
			                    {
			                    	for $PAYMENTS_TODAY in $ConsultaSaldosTarjetaCreditoRecordDetailDTO/ns2:PAYMENTS_TODAY
                                    return

...
declare namespace xf = "http://tempuri.org/Middleware/v2/Resources/ConsultaEstadoTransaccionCorr/xq/actualizarTrxGenericaCB/";

declare function xf:actualizarTrxGenericaCB($uuid as xs:string,
    $estado as xs:string,
    $FT24 as xs:string)
    as element(ns0:InputParameters) {
        <ns0:InputParameters>
            <ns0:PV_UUID_TRXS>{ $uuid }</ns0:PV_UUID_TRXS>
            <ns0:PV_REF_T24>{ $FT24 }</ns0:PV_REF_T24>
...
declare variable $FT24 as xs:string external;

xf:actualizarTrxGenericaCB($uuid,
    $estado,
    $FT24)
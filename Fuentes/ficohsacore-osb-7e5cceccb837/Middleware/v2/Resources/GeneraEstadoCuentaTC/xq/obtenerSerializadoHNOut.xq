(:: pragma bea:global-element-parameter parameter="$obtenerSerializadoResponse" element="ns0:ObtenerSerializadoResponse" location="../../../BusinessServices/VASA/obtenerSerializado/wsdl/ConsultaReporte.wsdl" ::)
...
declare function xf:obtenerSerializadoHNOut($obtenerSerializadoResponse as element(ns0:ObtenerSerializadoResponse))
    as element(ns1:generaEstadoCuentaTCResponse) {
        <ns1:generaEstadoCuentaTCResponse>
            {
                for $NumeroTarjeta in $obtenerSerializadoResponse/ns0:ObtenerSerializadoResult/ns0:NumeroTarjeta
                return
                    <CARD_NUMBER>{ data($NumeroTarjeta) }</CARD_NUMBER>
            }
            {
                for $NumeroCuenta in $obtenerSerializadoResponse/ns0:ObtenerSerializadoResult/ns0:NumeroCuenta
                return
                    <ACCOUNT_NUMBER>{ data($NumeroCuenta) }</ACCOUNT_NUMBER>
            }
            {
                for $Periodo in $obtenerSerializadoResponse/ns0:ObtenerSerializadoResult/ns0:Periodo
                return
            ...
            {
                for $File in $obtenerSerializadoResponse/ns0:ObtenerSerializadoResult/ns0:File
                return
                    <FILE_EXTRACT>{ data($File) }</FILE_EXTRACT>
            }
            {
                for $Extension in $obtenerSerializadoResponse/ns0:ObtenerSerializadoResult/ns0:Extension
                return

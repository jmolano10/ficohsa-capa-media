...
declare function xf:sjConsultaCuentaTCHNOut($resultGroup as element(*), $sjconsultaCuentaTC as element(ns0:sjconsultaCuentaTC))
    as element(ns0:sjconsultaCuentaTCResponse) {
        <ns0:sjconsultaCuentaTCResponse>
        {
			...
			return(
				for $account in $accountInquiryL8VLResponse/AZXAIO-ACCT
				 	...
				 	for $maxPaymentDate in $accountInquiryL8VLResponse/AZXAIO-DATE-PMT-DUE
				 	return
				 		if(fn-bea:trim(string($maxPaymentDate/text())) != '') then(
				 			<MAX_PAYMENT_DATE>{ data($maxPaymentDate) }</MAX_PAYMENT_DATE>
				 		) else(),
        ...
        {
		        let $datosCuenta := $sjconsultaCuentaTC/ACCOUNT_INFORMATION
	         	let $cashOTBResponse := $resultGroup/CASH_OTB_INQUIRY_RESPONSES/ns4:OTBResponse[1]
	 			let $accountInquiryL8VLResponse := $resultGroup/ACCOUNT_INQUIRY_RESPONSES/ns4:AccountInquiryL8VLResponse[1]
	 			let $balanceInquiryVAResponse := $resultGroup/BALANCE_INQUIRY_RESPONSES/ns4:BalanceInquiryResponse[1]
	        	return
        	 		<BALANCE_INFORMATION>
			            {

(:: pragma bea:global-element-parameter parameter="$consultarSaldosVasaResponse1" element="ns1:ConsultarSaldosVasaResponse" location="../../../transaccionesAlcance/wsdl/wsinfotarjetas.asmx.wsdl" ::)
...
declare namespace xf = "http://tempuri.org/Middleware/v2/BusinessServices/SJS/consultaTCCliente/xq/sjDatosTarjetaOut/";
        	...
        	if (fn:string($consultarSaldosVasaResponse1/ns1:ConsultarSaldosVasaResult/ns1:estatusConfirmacion/text()) = "00") then (
	            <ns0:RESPONSE_BODY>
	                ...
	                }
		            {
		            	...
		            	return
		            		if ($limites/ns1:moneda = 340) then 
		            		(
		            			<ns0:LCY_MIN_PAYMENT>{ data($limites/ns1:pagoMinimo) }</ns0:LCY_MIN_PAYMENT>,
								<ns0:LCY_TOTAL_PAYMENT>{ data($limites/ns1:pagoContado) }</ns0:LCY_TOTAL_PAYMENT>
		            ...
		            {
		            	let $limitCount := fn:count($consultarSaldosVasaResponse1/ns1:ConsultarSaldosVasaResult/ns1:saldos/ns1:ArrayOfStrucSaldosConsulta[ns1:limiteCredito != 0]/ns1:limiteCredito)
		            	return 
		            	if ( $limitCount = 0) then (
		            		<ns0:CREDIT_LIMIT>0</ns0:CREDIT_LIMIT>,
			            	<ns0:CREDIT_LIMIT_CCY>HNL</ns0:CREDIT_LIMIT_CCY>
			            ) else if ($limitCount = 1) then (
			            	<ns0:CREDIT_LIMIT>{ fn:string($consultarSaldosVasaResponse1/ns1:ConsultarSaldosVasaResult/ns1:saldos/ns1:ArrayOfStrucSaldosConsulta[ns1:limiteCredito != 0]/ns1:limiteCredito/text()) }</ns0:CREDIT_LIMIT>,
			            	<ns0:CREDIT_LIMIT_CCY>{ local:getMoneda(fn:string($consultarSaldosVasaResponse1/ns1:ConsultarSaldosVasaResult/ns1:saldos/ns1:ArrayOfStrucSaldosConsulta[ns1:limiteCredito != 0]/ns1:moneda/text())) }</ns0:CREDIT_LIMIT_CCY>
			            ) else (
			            	<ns0:CREDIT_LIMIT>{ fn:string($consultarSaldosVasaResponse1/ns1:ConsultarSaldosVasaResult/ns1:saldos/ns1:ArrayOfStrucSaldosConsulta[ns1:moneda = 840]/ns1:limiteCredito/text()) }</ns0:CREDIT_LIMIT>,
			            	<ns0:CREDIT_LIMIT_CCY>{ local:getMoneda(fn:string($consultarSaldosVasaResponse1/ns1:ConsultarSaldosVasaResult/ns1:saldos/ns1:ArrayOfStrucSaldosConsulta[ns1:moneda = 840]/ns1:moneda/text())) }</ns0:CREDIT_LIMIT_CCY>
			            )
		            }		                     
	                {
	                ...
	                }
	                {
	                    ...
	                    return
	                        <ns0:BALANCE_DETAILS>
	                            {
	                                for $ArrayOfStrucSaldosConsulta in $saldos/ns1:ArrayOfStrucSaldosConsulta
	                                return
	                                    <ns0:BALANCE_DETAIL>

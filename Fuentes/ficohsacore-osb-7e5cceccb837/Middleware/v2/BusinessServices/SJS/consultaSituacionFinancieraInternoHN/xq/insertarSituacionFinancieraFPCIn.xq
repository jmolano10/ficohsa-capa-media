(:: pragma bea:global-element-parameter parameter="$outputParameters" element="ns1:OutputParameters" location="../../../FPC/consultaSituacionFinanciera12c/xsd/GetFinancialSituationTypes.xsd" ::)
...
declare function xf:insertarSituacionFinancieraFPCIn($outputParameters as element(ns1:OutputParameters),
    ...
    as element(ns0:InputParameters) {
        <ns0:InputParameters>
            ...
            {
                for $PV_BANCO_REGION_OUT in $outputParameters/PV_BANCO_REGION_OUT
                return
                    <ns0:PV_REGION>{ data($PV_BANCO_REGION_OUT) }</ns0:PV_REGION>
            }
            <ns0:PV_USUARIO_SERVICIO>{ $usuario }</ns0:PV_USUARIO_SERVICIO>
            {
            	if(fn:exists($outputParameters/PT_SALDO_TOTAL/PT_SALDO_TOTAL_ITEM)) then (
            		<ns0:PT_APLICACION_CORE>
            		{
            			for $i in 1 to count($outputParameters/PT_SALDO_TOTAL/PT_SALDO_TOTAL_ITEM)
                    	return(
            ...
            {
            	if(fn:exists($outputParameters/PT_CODIGO_MONEDA/PT_CODIGO_MONEDA_ITEM)) then (
            		<ns0:PT_CODIGO_MONEDA>
            		{
            			for $Currency in $outputParameters/PT_CODIGO_MONEDA/PT_CODIGO_MONEDA_ITEM
                    	return(
                        	<ns0:PT_CODIGO_MONEDA_ITEM>{ fn:data($Currency) }</ns0:PT_CODIGO_MONEDA_ITEM>
                    	)
            		}
            		</ns0:PT_CODIGO_MONEDA>
            	) else ()
            }
            {
            	if(fn:exists($outputParameters/PT_SALDO_TOTAL/PT_SALDO_TOTAL_ITEM)) then (
                	...
                	{
                    	for $PT_SALDO_TOTAL_ITEM in $outputParameters/PT_SALDO_TOTAL/PT_SALDO_TOTAL_ITEM
                    	return(
                    		<ns0:PT_SALDO_DISPONIBLE_ITEM>{ data($PT_SALDO_TOTAL_ITEM) }</ns0:PT_SALDO_DISPONIBLE_ITEM>
                    	)
                	}
            		</ns0:PT_SALDO_DISPONIBLE>,
            		<ns0:PT_SALDO_TOTAL>
            ...
            }
            {
            	if(fn:exists($outputParameters/PT_SALDO_TOTAL/PT_SALDO_TOTAL_ITEM)) then (
            		<ns0:PT_SUCCESS_INDICADOR_IN>
                	{
                    	for $i in 1 to count($outputParameters/PT_SALDO_TOTAL/PT_SALDO_TOTAL_ITEM)
                    	return(
                        	<ns0:PT_APLICACION_CORE_ITEM>{ fn:data($outputParameters/PV_CODIGO_ERROR) }</ns0:PT_APLICACION_CORE_ITEM>
                    	)
                	}
            		</ns0:PT_SUCCESS_INDICADOR_IN>
            	) else ()
           }
           {
           		if(fn:exists($outputParameters/PT_SALDO_TOTAL/PT_SALDO_TOTAL_ITEM)) then (
           			<ns0:PT_DESCRIPCION_MENSAJE>
                	{
                    	for $i in 1 to count($outputParameters/PT_SALDO_TOTAL/PT_SALDO_TOTAL_ITEM)
                    	return(
                        	<ns0:PT_DESCRIPCION_MENSAJE_ITEM>{ fn:string($outputParameters/PV_DESCRIPCION_ERROR/text()) }</ns0:PT_DESCRIPCION_MENSAJE_ITEM>
                        )
                	}
            		</ns0:PT_DESCRIPCION_MENSAJE>
           		) else ()

...
declare function xf:registraMestroSaldosTCNIIn($outputParameters as element(ns1:OutputParameters),
                    ...
                    <ns0:CARD_NUMBER>{ $cardNumer }</ns0:CARD_NUMBER>
                      {
						...
						return
							if($valFechaPagoMaximo != '')then(
								<ns0:MAX_PAYMENT_DATE>{$valFechaPagoMaximo}</ns0:MAX_PAYMENT_DATE>
							) else(),
...
xf:registraMestroSaldosTCNIIn($outputParameters,

...
(:: pragma bea:global-element-parameter parameter="$outputParameters" element="ns1:OutputParameters" location="../../../../v2/BusinessServices/ProcesosPA/conMaestroSaldos/xsd/ConsultaMaestroSaldosTarjetaCredito_sp.xsd" ::)
...
declare namespace xf =  "http://tempuri.org/Middleware/v3/Resources/ConsultaMaestraSaldosTarjetaCredito/xq/consultaMaestroSaldosTarjetaCreditoPAOut/";
...
declare function xf:consultaMaestraSaldosTarjetaCreditoOut($sjConsultaMaestraSaldosTarjetaCreditoResponse as element(ns6:sjConsultaMaestraSaldosTarjetaCreditoResponse)?,
    $outputParameters as element(ns1:OutputParameters)?,
    $valSaldoActualPuntos as  xs:string?, 
    $valDisponibleCompras as xs:string?,
    	...
    	<ns0:consultaMaestraSaldosTarjetaCreditoResponse>
		{
			if(not(empty($outputParameters1/ns2:PT_MAESTRA_SALDOS_TC/ns2:PT_MAESTRA_SALDOS_TC_ITEM)))then(
	        	...
	        	let $valFechaPagoMaximo := (
		        	for $item in data($outputParameters/ns1:RowSet/ns1:Row[1]/ns1:Column[@name = 'fecpagodue'])
		        	where not($item = ('','00000000'))
						return $item
				) return
					if(exists($valFechaPagoMaximo)) then (
						<MAX_PAYMENT_DATE>{fn-bea:dateTime-to-string-with-format('dd/MM/yyyy', $valFechaPagoMaximo[1])}</MAX_PAYMENT_DATE>
					) else(),
	        	...
	        	let $valfechaCorte := string($sjConsultaMaestraSaldosTarjetaCreditoResponse/CUTOFF_DATE/text())
	        	 ...
	        	 return(
	        		<CREDIT_CARD_BALANCE_INFO>
	        		{
	        	 	...
	        	 	{
	        	 		for $valSaldoActual in string($rowconSaldosCTS/ns1:Column[@name = 'billed_bal']/text() - $sjConsultaMaestraSaldosTarjetaCredito/PAYMENT_TODAY/text())
	        	 		return
	        	...
	        	let $intra_end_bal := xs:decimal($outputParameters/ns1:RowSet/ns1:Row[1]/ns1:Column[@name = 'intra_end_bal']/text())
	    		let $totalPayment := xs:decimal($sjConsultaMaestraSaldosTarjetaCreditoResponse/TOTAL_PAYMENT)
	    		let $valPagoContadoTotal :=  $intra_end_bal - $totalPayment
	    		return(
...
declare variable $valDisponibleCompras as xs:string? external;
declare variable $outputParameters1 as element(ns2:OutputParameters)? external;

xf:consultaMaestraSaldosTarjetaCreditoOut($sjConsultaMaestraSaldosTarjetaCreditoResponse,
    $outputParameters, $valSaldoActualPuntos, $valDisponibleCompras, $outputParameters1)
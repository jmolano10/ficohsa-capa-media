...
(:: pragma bea:global-element-parameter parameter="$outputParameters" element="ns1:OutputParameters" location="../../../../v2/BusinessServices/MasterDataGT/conMaestroSaldos/xsd/ConsultaMaestroSaldosTarjetaCredito_sp.xsd" ::)
...
declare namespace xf = "http://tempuri.org/Middleware/v3/Resources/ConsultaMaestraSaldosTarjetaCredito/xq/consultaMaestroSaldosTarjetaCreditoGTOut/";
        ...
        <ns0:consultaMaestraSaldosTarjetaCreditoResponse>
        {
   			if(not(empty($outputParameters1/ns2:PT_MAESTRA_SALDOS_TC/ns2:PT_MAESTRA_SALDOS_TC_ITEM)))then(
			    ...
			    if ($fechaMaximaPago != '') then (
					<MAX_PAYMENT_DATE>{ date-to-string($fechaMaximaPago) }</MAX_PAYMENT_DATE>
		    	) else if (data($sjConsultaMaestraSaldosTarjetaCreditoResponse/MAX_PAYMENT_DATE) != '') then (
		    		<MAX_PAYMENT_DATE>{ data($sjConsultaMaestraSaldosTarjetaCreditoResponse/MAX_PAYMENT_DATE) }</MAX_PAYMENT_DATE>
		    	) else (),
				...
				return(
					if(string($valPagoContadoTotal) != '')then(
						if($valPagoContadoTotal > 0)then(
    	 					<ns0:TOTAL_PAYMENT>{ fn-bea:decimal-round(xs:decimal($valPagoContadoTotal), 2) }</ns0:TOTAL_PAYMENT>
    	 				)else(
    	 					<ns0:TOTAL_PAYMENT>0</ns0:TOTAL_PAYMENT>
    	 				)
    	 			)else(
    	 				<ns0:TOTAL_PAYMENT>{ $valPagoContado - $totalPayment }</ns0:TOTAL_PAYMENT>
    	 			)
				)
   			)
   		}  

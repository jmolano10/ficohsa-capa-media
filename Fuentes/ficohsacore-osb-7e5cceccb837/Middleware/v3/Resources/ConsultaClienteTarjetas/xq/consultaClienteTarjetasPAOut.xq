...
declare namespace xf = "http://tempuri.org/Middleware/v3/Resources/ConsultaClienteTarjetas/xq/consultaClienteTarjetasPAOut/";
	        	...
	        	return(
	        		...
	        		<addresInformationResidence>
	        			...
	        			<ADDRESS_LINE_4>{ data($demographic/AZXNDO-ADDR-4) }</ADDRESS_LINE_4>
	        			{
		        			...
		        			return(
		        				<CITY>{ valorCatalogoAPPToOSB(string($CITY/text()), 'VISION+PA', '7', $ptEquivalencias) }</CITY>
		        			)
	       				}
	       				{
		        			for $STATE in $demographic/AZXNDO-CITY-STATE
		        			return(
		        				<STATE>{ valorCatalogoAPPToOSB(string($STATE/text()), 'VISION+PA', '8', $ptEquivalencias) }</STATE>
		        			)
	       				}
	       				{
		        			for $COUNTRY in $demographic/AZXNDO-CNTRY-CD
		        			return(
		        				<COUNTRY>{ valorCatalogoAPPToOSB(string($COUNTRY/text()), 'VISION+PA', '9', $ptEquivalencias) }</COUNTRY>
		        			)
	       				}           			
	        		...
	        		return(
	        			<MARITAL_STATUS>{ valorCatalogoAPPToOSB(string($MARITAL_STATUS/text()), 'MASTERDATAPA', '2', $ptEquivalencias) }</MARITAL_STATUS>
	        		),       		
	        		
	        		for $LEGAL_DOCUMENT_TYPE in $demographic/AZXNDO-SSAN-FLAG
	        		return(
	        			<LEGAL_DOCUMENT_TYPE>{ valorCatalogoAPPToOSB(string($LEGAL_DOCUMENT_TYPE/text()), 'VISION+PA', '4', $ptEquivalencias) }</LEGAL_DOCUMENT_TYPE>
	        		),
					...
					return(
						<TRAVELERS_STATUS>{ valorCatalogoAPPToOSB(string($TRAVELERS_STATUS/text()), 'MASTERDATAPA', '5', $ptEquivalencias) }</TRAVELERS_STATUS>
					),				
					...
					return(
						<EDUCATION_LEVEL>{ valorCatalogoAPPToOSB(string($EDUCATION_LEVEL/text()), 'VISION+PA', '10', $ptEquivalencias) }</EDUCATION_LEVEL>
					),
					
					for $GENDER in $demographic/AZXNDO-GENDER-CODE
					return(
						<GENDER>{ valorCatalogoAPPToOSB(string($GENDER/text()), 'VISION+PA', '1', $ptEquivalencias) }</GENDER>				
					),
					...
					<COUNTRY_ISSUANCE_PASSAPORT>{ data($demographic/AZXNDO-DL-CNTRY) }</COUNTRY_ISSUANCE_PASSAPORT>,	

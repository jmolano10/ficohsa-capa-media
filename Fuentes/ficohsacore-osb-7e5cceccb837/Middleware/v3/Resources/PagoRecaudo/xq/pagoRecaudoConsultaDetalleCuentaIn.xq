...
declare namespace xf = "http://tempuri.org/Middleware/v3/Resources/PagoRecaudo/xq/pagoRecaudoConsultaDetalleCuentaIn/";
...
declare function xf:pagoRecaudoConsultaDetalleCuentaIn($requestHeader as element(ns0:RequestHeader),
    $cuentaTemp as xs:string)
    as element(ns1:Consultadedetallesdelacuenta) {
        <ns1:Consultadedetallesdelacuenta>
            <WebRequestCommon>
                <userName>{ fn-bea:fail-over( getUsername(data($requestHeader/Authentication/UserName)), 
												data($requestHeader/Authenticatiom/UserName)) }</userName>
                <password>{ fn-bea:fail-over( getPassword(data($requestHeader/Authentication/UserName)),
                								data($requestHeader/Authenticatiom/Password)) }</password>
            </WebRequestCommon>
            <WSACCOUNTLISTType>
...
xf:pagoRecaudoConsultaDetalleCuentaIn($requestHeader,
    $cuentaTemp)
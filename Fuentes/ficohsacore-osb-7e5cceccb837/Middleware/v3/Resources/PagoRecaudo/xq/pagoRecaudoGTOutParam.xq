...
declare namespace xf = "http://tempuri.org/Middleware/v3/Resources/PagoRecaudo/xq/pagoRecaudoGTOutParam/";

declare function xf:pagoRecaudoGTOutParam($outputParameters1 as element(ns1:OutputParameters),
    $outputParameters2 as element(ns0:OutputParameters),
    $paymentMethod as xs:string,
    $codigo_contrato as xs:string)
    as element(ns2:pagoRecaudoResponse) {
        <ns2:pagoRecaudoResponse>
            <CONTRACT_NAME>{ $codigo_contrato }</CONTRACT_NAME>
            {
                let $result :=
...
declare variable $codigo_contrato as xs:string external;

xf:pagoRecaudoGTOutParam($outputParameters1,
    $outputParameters2,
    $paymentMethod,
    $codigo_contrato)
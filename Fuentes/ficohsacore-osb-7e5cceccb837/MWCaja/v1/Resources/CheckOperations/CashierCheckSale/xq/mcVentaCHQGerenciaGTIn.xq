(:: pragma bea:global-element-parameter parameter="$cashierCheckSale" element="ns2:cashierCheckSale" location="../../xsd/CheckOperationsTypes.xsd" ::)
(:: pragma bea:global-element-return element="ns0:InputParameters" location="../../../../BusinessServices/AbanksGT/VentaCHQGerencia/xsd/MCVentaCHQGerencia_sp.xsd" ::)
...
declare namespace xf = "http://tempuri.org/MWCaja/v1/Resources/CheckOperations/CashierCheckSale/xq/mcVentaCHQGerenciaGTIn/";

declare function xf:mcVentaCHQGerenciaGTIn($cashierCheckSale as element(ns2:cashierCheckSale))
    as element(ns0:InputParameters) {
        <ns0:InputParameters>
            <ns0:PN_CHECK_NUMBER>{ data($cashierCheckSale/ns2:CHECK_NUMBER) }</ns0:PN_CHECK_NUMBER>
            <ns0:PV_BENEFICIARY_NAME>{ data($cashierCheckSale/ns2:BENEFICIARY_NAME) }</ns0:PV_BENEFICIARY_NAME>
            <ns0:PN_CHECK_AMOUNT>{ data($cashierCheckSale/ns2:CHECK_AMOUNT) }</ns0:PN_CHECK_AMOUNT>
            <ns0:PN_COMMISSION_AMOUNT>{ data($cashierCheckSale/ns2:COMMISSION_AMOUNT) }</ns0:PN_COMMISSION_AMOUNT>
            {
                let $PAYMENT := $cashierCheckSale/ns2:PAYMENT
                return
                    <ns0:PR_PAYMENT>
                        <ns0:PAY_ITEM>
                            {
                                ...
                                return
                                    <ns0:PAY_ITEM_ITEM>
                                        <ns0:TYPE>{ data($PAY_ITEM/ns1:TYPE) }</ns0:TYPE>
            ...
            {
                let $NSO_INFO := $cashierCheckSale/ns2:NSO_INFO
                return
                    <ns0:PR_NSO_INFO>
                        {
                        ...
                        }
                        {
                        	...
                        	return
                        		 <ns0:CTR_REFERENCE>{ data($NSO_INFO/ns1:CTR_REFERENCE) }</ns0:CTR_REFERENCE>
                        }
...
declare variable $cashierCheckSale as element(ns2:cashierCheckSale) external;

xf:mcVentaCHQGerenciaGTIn($cashierCheckSale)

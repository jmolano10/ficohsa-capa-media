(:: pragma bea:global-element-parameter parameter="$tillDifferenceReport" element="ns0:tillDifferenceReport" location="../../xsd/TillOperationsTypes.xsd" ::)
(:: pragma bea:global-element-return element="ns1:InputParameters" location="../../../../BusinessServices/AbanksGT/REFDiferencia/xsd/MCREFDiferencia_sp.xsd" ::)
...
declare namespace xf = "http://tempuri.org/MWCaja/v1/Resources/TillOperations/TillDifferenceReport/xq/mcREFDiferenciaGTIn/";

declare function xf:mcREFDiferenciaGTIn($tillDifferenceReport as element(ns0:tillDifferenceReport))
    as element(ns1:InputParameters) {
        <ns1:InputParameters>
            <ns1:PV_REPORT_TYPE>{ data($tillDifferenceReport/ns0:REPORT_TYPE) }</ns1:PV_REPORT_TYPE>
            <ns1:PV_REPORT_SOURCE>{ data($tillDifferenceReport/ns0:REPORT_SOURCE) }</ns1:PV_REPORT_SOURCE>
            <ns1:PD_DATE>{ data($tillDifferenceReport/ns0:DATE) }</ns1:PD_DATE>
            {
                let $PAYMENT := $tillDifferenceReport/ns0:PAYMENT
                return
                    <ns1:PR_PAYMENT>
                        <ns1:PAY_ITEM>
                            {
                                for $PAY_ITEM in $PAYMENT/ns2:PAY_ITEM
                                return
                                    <ns1:PAY_ITEM_ITEM>
            ...
            {
                let $NSO_INFO := $tillDifferenceReport/ns0:NSO_INFO
                return
                    <ns1:PR_NSO_INFO>
                        {
...
declare variable $tillDifferenceReport as element(ns0:tillDifferenceReport) external;

xf:mcREFDiferenciaGTIn($tillDifferenceReport)

(:: pragma bea:global-element-parameter parameter="$fixedTermInitialDeposit" element="ns2:fixedTermInitialDeposit" location="../../xsd/FixedTermDepOperationsTypes.xsd" ::)
(:: pragma bea:global-element-return element="ns0:InputParameters" location="../../../../BusinessServices/AbanksGT/DepositoInicial/xsd/MCDepositoCD_sp.xsd" ::)
...
declare namespace xf = "http://tempuri.org/MWCaja/v1/Resources/FixedTermDepOperations/FixedTermInitialDeposit/xq/mcDepositoCDGTIn/";

declare function xf:mcDepositoCDGTIn($fixedTermInitialDeposit as element(ns2:fixedTermInitialDeposit))
    as element(ns0:InputParameters) {
        <ns0:InputParameters>
            <ns0:PN_ACCOUNT_NUMBER>{ xs:decimal(data($fixedTermInitialDeposit/ns2:ACCOUNT_NUMBER)) }</ns0:PN_ACCOUNT_NUMBER>
            {
                let $DEPOSIT_ITEMS := $fixedTermInitialDeposit/ns2:DEPOSIT_ITEMS
                return
                    <ns0:PR_DEPOSIT_ITEMS>
                        <ns0:PAY_ITEM>
                            {
                                for $DEPOSIT_ITEM in $DEPOSIT_ITEMS/ns2:DEPOSIT_ITEM
                                return
                                    <ns0:PAY_ITEM_ITEM>
                                        <ns0:TYPE>{ data($DEPOSIT_ITEM/ns1:TYPE) }</ns0:TYPE>
                                        <ns0:CURRENCY>{ data($DEPOSIT_ITEM/ns1:CURRENCY) }</ns0:CURRENCY>
                                        {
                                            for $AMOUNT in $DEPOSIT_ITEM/ns1:AMOUNT
                                            return
                                                <ns0:AMOUNT>{ data($AMOUNT) }</ns0:AMOUNT>
                                        }
                                        {
                                            for $MORE_INFO in $DEPOSIT_ITEM/ns1:MORE_INFO
                                            return
                                                <ns0:MORE_INFO>
                                                    {
                                                        for $ACCOUNT in $MORE_INFO/ns1:ACCOUNT
                                                        return
                                                            <ns0:ACCOUNT_VNUMBER>{ data($ACCOUNT /ns1:NUMBER) }</ns0:ACCOUNT_VNUMBER>
                                                    }
                                                    {
                                                        for $CHECK in $MORE_INFO/ns1:CHECK
                                                        return
                                                            <ns0:CHECK_CHECK_NUMBER>{ data($CHECK/ns1:CHECK_NUMBER) }</ns0:CHECK_CHECK_NUMBER>
                                                    }
                                                    {
                                                        for $CHECK in $MORE_INFO/ns1:CHECK,
            ...
            {
                let $NSO_INFO := $fixedTermInitialDeposit/ns2:NSO_INFO
                return
                    <ns0:PR_NSO_INFO>
                        {
                                    ...
                                    <ns0:BANKNOTE>
                                        {
                                            ...
                                            return
                                                <ns0:BANKNOTE_ITEM>
                                                    ...
                                                    <ns0:QUANTITY>{ data($BANKNOTE/ns1:QUANTITY) }</ns0:QUANTITY>
                        ...
                        }
                        {
                        	...
                        	return
                        		<ns0:CTR_REFERENCE>{ data($CTR_REFERENCE) }</ns0:CTR_REFERENCE>
                        }
                    </ns0:PR_NSO_INFO>
            }
        </ns0:InputParameters>
...
declare variable $fixedTermInitialDeposit as element(ns2:fixedTermInitialDeposit) external;

xf:mcDepositoCDGTIn($fixedTermInitialDeposit)

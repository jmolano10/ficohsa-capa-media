...
(:: pragma bea:global-element-return element="ns0:InputParameters" location="../../../../../v1/BusinessServices/AbanksGT/REPPagoTC/xsd/MCREPPagoTC_sp.xsd" ::)
...
declare namespace xf = "http://tempuri.org/MWCaja/v1/Resources/CreditCardOperations/CreditCardPayment/xq/mcREPPagoTCGTIn/";
declare namespace ns3 = "http://www.ficohsa.com.hn/middleware.services/creditCardTypes";

declare function xf:mcREPPagoTCGTIn($creditCardPayment as element(ns2:creditCardPayment))
    as element(ns0:InputParameters) {
        <ns0:InputParameters>
            <ns0:PN_CREDIT_CARD_NUMBER>{ xs:decimal(data($creditCardPayment/ns2:CREDIT_CARD_NUMBER)) }</ns0:PN_CREDIT_CARD_NUMBER>
            {
                let $PAYMENT_TARGET := $creditCardPayment/ns2:PAYMENT_TARGET
                return
                    <ns0:PR_PAYMENT_TARGET>
                        <ns0:CREDIT_LINE>{ data($PAYMENT_TARGET/ns3:CREDIT_LINE) }</ns0:CREDIT_LINE>
                        {
                            for $REFERENCE in $PAYMENT_TARGET/ns3:REFERENCE
                            return
                                <ns0:REFERENCE>{ data($REFERENCE) }</ns0:REFERENCE>
                        }
                    </ns0:PR_PAYMENT_TARGET>
            }
            <ns0:PV_BALANCE_CURRENCY>{ data($creditCardPayment/ns2:BALANCE_CURRENCY) }</ns0:PV_BALANCE_CURRENCY>
            {
                let $PAYMENT := $creditCardPayment/ns2:PAYMENT
                return
                    <ns0:PR_PAYMENT>
                        <ns0:PAY_ITEM>
                            {
            ...
            {
                let $NSO_INFO := $creditCardPayment/ns2:NSO_INFO
                return
                    <ns0:PR_NSO_INFO>
                        {
...
xf:mcREPPagoTCGTIn($creditCardPayment)

(:: pragma bea:global-element-parameter parameter="$ccFinanceDisbursement" element="ns2:ccFinanceDisbursement" location="../../../../../v1/Resources/CreditCardOperations/xsd/CreditCardOperationsTypes.xsd" ::)
(:: pragma bea:global-element-return element="ns0:InputParameters" location="../../../../../v1/BusinessServices/AbanksGT/REPDesembTC/xsd/MCREPDesembTC_sp.xsd" ::)
...
declare namespace xf = "http://tempuri.org/MWCaja/v1/Resources/CreditCardOperations/CcFinanceDisbursement/xq/mcREPDesembTCGTIn/";
declare namespace ns3 = "http://www.ficohsa.com.hn/middleware.services/creditCardTypes";

declare function xf:mcREPDesembTCGTIn($ccFinanceDisbursement as element(ns2:ccFinanceDisbursement))
    as element(ns0:InputParameters) {
        <ns0:InputParameters>
            <ns0:PN_CREDIT_CARD_NUMBER>{ xs:decimal(data($ccFinanceDisbursement/ns2:CREDIT_CARD_NUMBER)) }</ns0:PN_CREDIT_CARD_NUMBER>
            <ns0:PV_EXPIRATION_DATE>{ data($ccFinanceDisbursement/ns2:EXPIRATION_DATE) }</ns0:PV_EXPIRATION_DATE>
            {
                let $FINANCE_DETAILS := $ccFinanceDisbursement/ns2:FINANCE_DETAILS
                return
                    <ns0:PR_FINANCE_DETAILS>
                        <ns0:CREDIT_LINE>{ data($FINANCE_DETAILS/ns3:CREDIT_LINE) }</ns0:CREDIT_LINE>
                        {
                            for $CURRENCY in $FINANCE_DETAILS/ns3:CURRENCY
                            return
                                <ns0:CURRENCY>{ data($CURRENCY) }</ns0:CURRENCY>
                        }
                        {
                            for $AMOUNT in $FINANCE_DETAILS/ns3:AMOUNT
                            return
                                <ns0:AMOUNT>{ data($AMOUNT) }</ns0:AMOUNT>
                        }
                        {
                            for $MONTHS in $FINANCE_DETAILS/ns3:MONTHS
                            return
                                <ns0:MONTHS>{ xs:decimal(data($MONTHS)) }</ns0:MONTHS>
                        }
                    </ns0:PR_FINANCE_DETAILS>
            }
            {
                let $PAYMENT := $ccFinanceDisbursement/ns2:PAYMENT
                return
                    <ns0:PR_PAYMENT>
                        <ns0:PAY_ITEM>
                            {
            ...
            {
                let $NSO_INFO := $ccFinanceDisbursement/ns2:NSO_INFO
                return
                    <ns0:PR_NSO_INFO>
                        {
...
declare variable $ccFinanceDisbursement as element(ns2:ccFinanceDisbursement) external;

xf:mcREPDesembTCGTIn($ccFinanceDisbursement)

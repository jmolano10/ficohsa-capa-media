xquery version "1.0" encoding "utf-8";

(:: OracleAnnotationVersion "1.0" ::)

declare namespace ns1="http://www.ficohsa.com.hn/middleware.services/pagosMasivosTypes";
(:: import schema at "../../../Schemas/PagosMasivos/pagosMasivosTypes.xsd" ::)
declare namespace ns2="https://www.ficohsa.com/regional/massivePayment";
(:: import schema at "../../../../SBRG_MassivePayment_CancelBatch/Schemas/CancelBatchTypes.xsd" ::)

declare variable $cancelarLote as element() (:: schema-element(ns1:cancelarLote) ::) external;
declare variable $SourceBank as xs:string external;
declare variable $DestinationBank as xs:string external;

declare function local:func(
    $cancelarLote as element() (:: schema-element(ns1:cancelarLote) ::),
    $SourceBank as xs:string, 
    $DestinationBank as xs:string
    ) as element() (:: schema-element(ns2:cancelBatch) ::) {
    <ns2:cancelBatch>
        <GeneralInfo>
            <SourceBank>{fn:data($SourceBank)}</SourceBank>
            <DestinationBank>{fn:data($DestinationBank)}</DestinationBank>
        </GeneralInfo>
        <CustomerId>{fn:data($cancelarLote/CUSTOMER_ID)}</CustomerId>
        <UserId>{fn:data($cancelarLote/USER_ID)}</UserId>
        <QueryType>BANK_BATCH_ID</QueryType>
        <QueryValue>{fn:data($cancelarLote/BANK_BATCH_ID)}</QueryValue>
    </ns2:cancelBatch>
};

local:func($cancelarLote, $SourceBank, $DestinationBank)
...
declare function local:coreToRegional($outstandingAuthorizationInquiryResponse as element() (:: schema-element(ns1:OutstandingAuthorizationInquiryResponse) ::), 
  ...
  <OwnerName>{fn-bea:trim(fn:replace(fn:string(local:replace-first($cardHolderName, '0', '-')),"-", " "))}</OwnerName>
  ...
  <TotalUsdAmount>{if ($usdAmmount = '0') then '0.00' else fn-bea:format-number(number($usdAmmount),"##0.00")}</TotalUsdAmount>
    <CardsDetails>
      <CardNumber>{ 
        ...
        return if ($cardNumber  = '') then (fn-bea:trim(fn:replace(fn:string(local:replace-first(data($usdmovements/OutstandingAuthArray/OutstandingAuthData[1]/OFXOAO-CARD-NBR), '0', '-')),"-", " "))) else $cardNumber   
      }</CardNumber>

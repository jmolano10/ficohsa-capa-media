...
declare function local:funcRollbackPurchaseToPagosWS($Rollback as element() (:: schema-element(ns1:getStatusTicket) ::)) as element() (:: schema-element(ns2:InputParameters) ::) {
    <ns2:InputParameters>
        <PV_TICKET>{fn:data($Rollback/Ticket)}</PV_TICKET>
    </ns2:InputParameters>
};

local:funcRollbackPurchaseToPagosWS($Rollback)
...
declare namespace acc="https://www.ficohsa.com/regional/card";
...
declare function local:funcErrorToRegional($responseError as element() (:: schema-element(ns1:errorMappingRegionalOutput) ::), 
                                                    $codeError as xs:string, 
                                                    $messageError as xs:string, 
                                                    $targetSystem as xs:string, 
                                                    $status as xs:string, 
                                                    $globalId as xs:string) as element() (:: schema-element(acc:getDebitCardsByCustomerResponse) ::) {
    <acc:getDebitCardsByCustomerResponse> 
     <StatusInfo>
...
local:funcErrorToRegional($responseError, $codeError, $messageError, $targetSystem, $status, $globalId)
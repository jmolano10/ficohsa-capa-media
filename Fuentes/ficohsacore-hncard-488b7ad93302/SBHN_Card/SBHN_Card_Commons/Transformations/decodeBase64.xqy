...
declare function local:index-of-string( $arg as xs:string? ,
...
declare function local:decodeLoginFromBase64($entrada as xs:string) as xs:string {
    let $decodeUser := substring($entrada, 7)
    
    let $credentials := fn:codepoints-to-string(local:base64-decode( $decodeUser )) 
    
    let $username := tokenize($credentials, ':')[position() = 1]
...
local:decodeLoginFromBase64($entrada)